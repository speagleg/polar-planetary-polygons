...
the Cabibbo--Kobayashi--Maskawa (CKM) phase, three generations of fermions,
...
derives the $E_8 \to \text{Standard Model}$ curvature transition,
and provides the ultraviolet (high-energy) completion.
...
Metric-independence of $f(m,N) = m(N{-}m)/2$
  ...
  Thm~I-3.2 & I \\[4pt]
$\mathbf{S}^2$ thresholds are Pell units:
  $\xi_{\mathrm{crit}} \in \{1,\, 2{-}\sqrt{3},\, 3{-}2\sqrt{2},\, 9{-}4\sqrt{5}\}$ &
  eq.~I-(4.4) & I \\[4pt]
$\mathbf{H}^2$ and $\mathbf{S}^2$ thresholds share Pell structure in
  quadratic fields &
  Prop~\ref*{I-prop:alg-int} & I \\[4pt]
...
Universality: $N_{\mathrm{crit}} = 7$ at $\xi = 0$
  ...
  Thm~\ref*{I-thm:universality} & I \\[4pt]
...
Quartic normal form: $\alpha_0 = 45/14$
  ...
  \S II-5 & II \\[4pt]
Calogero--Moser--Sutherland (CMS) to
  Chern--Simons (CS) Casimir identification:
  $C_2^{\mathrm{CMS}} = C_2^{\mathrm{CS}}$ via
  symmetric-space Laplacian &
  Prop~\ref*{III-prop:casimir-havelock} & III \\[4pt]
Polygon entropy maximization implies
  $R = \mathrm{const}$ (Einstein equations) &
  Thm~\ref*{III-thm:polygon-einstein} & III \\[4pt]
\bottomrule
\end{tabular}
...
Graviton emerges at $N = 7$ as a boundary
  ...
  Prop~\ref*{III-prop:graviton} & III \\[4pt]
Vacuum Einstein equations from polygon
  entropy maximization (Jensen inequality;
  all conditions are theorems) &
  Thm~\ref*{III-thm:polygon-einstein} & III \\[4pt]
$\mathrm{SU}(3) \times \mathrm{SU}(2) \times
  \mathrm{U}(1)$ from $N = 7$ orbifold +
  McKay + CS gravity &
  Derivations~\ref*{IV-thm:su2}, \ref*{IV-thm:su3} & IV \\[4pt]
Weinberg angle $\sin^2\theta_W = 3/11$
  from CS conformal weight ratio &
  Derivation~\ref*{IV-thm:weinberg} & IV \\[4pt]
CKM phase $\delta = 69.3^\circ$ from
  BF-crossing scattering phase on
  $\mathbf{H}^2$ &
  Thm~IV-14.1 & IV \\[4pt]
$N = 11$ uniquely selected by Kaluza--Klein (KK) flux
  additivity; $H_0 = 67.4$~km/s/Mpc &
  Thm~VI-3.1 & VI \\[4pt]
Dark matter from frozen KK polygon modes
  ($50$--$950$~TeV, absolutely stable) &
  \S VI-5 & VI \\[4pt]
Baryogenesis: $\eta_B = 5.7 \times 10^{-10}$,
  $v_w$-independent &
  \S VI-7 & VI \\[4pt]
\bottomrule
\end{tabular}
...
Pell equation governs both vortex stability
  ...
  Prop~\ref*{III-prop:graviton} & I, III \\[4pt]
...
Onsager negative-temperature condensation
  $=$ Wheeler--DeWitt (WDW) quantum cosmology
  (Feynman--Kac identity) &
  \S III-13 & III \\[4pt]
McKay correspondence $+$
  Dixon--Harvey--Vafa--Witten (DHVW) orbifold
  $\to$ $\mathrm{SU}(3)_1$ gauge sector &
  Derivation~\ref*{IV-thm:su3} & IV \\[4pt]
\bottomrule
\end{tabular}
...
Chern--Simons / Wess--Zumino--Witten (WZW) correspondence &
  Witten (1989) \\[3pt]
...
$\theta_{\mathrm{QCD}} = 0$ (the strong-force vacuum angle).
Nine binary tests across five experimental programs
will resolve these within two decades.
...
II (Physics) & Paper I + physical constraints &
  ...
  & & blob/Rossby bridges, Bose--Einstein condensate/planetary predictions \\[3pt]
