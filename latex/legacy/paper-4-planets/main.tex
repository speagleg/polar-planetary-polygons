...
\title{\seriestitle\\[6pt]\large Paper V: Planetary and Astrophysical Applications}
...
This follows from the generalized Casimir
$h(m, N, \Delta)$ of Paper~III,
Proposition~\ref{III-prop:generalized-casimir}:
\begin{center}
\small
\begin{tabular}{lcccl}
\toprule
...
stabilizing curvature correction, making the vortex polygon
...
%% ====================================================================
\section{Predictions from the field theory (Paper~IV)}
\label{sec:field-theory-predictions}

The Havelock field theory (Paper~IV) gives the exact central
charge $c = 12\,b(N)$ and the stability thresholds on~$\mathbf{H}^2$,
refining the planetary predictions and extending them to cosmology.

\paragraph{Saturn's unconditional stability.}
On the flat plane, the hexagon ($N = 6$) has
$\lambda_3 = (N{-}1) - f(3,6) = 5 - 4.5 = 0.5 > 0$.
On $\mathbf{H}^2$ at \emph{any} positive curvature~$\xi$:
$C_1(\xi) = 5(1{+}\xi^2)/(1{-}\xi)^2 > 5 > f(3,6) = 4.5$,
so $\lambda_3 > 0$ for all $\xi > 0$.
The hexagon is \emph{unconditionally stable}---there is no
curvature threshold to cross.
This explains its persistence over four Saturnian decades of observation.

\paragraph{Jupiter's octagon threshold.}
For $N = 8$: $\lambda_4 = 7 - 8 = -1 < 0$ on the flat plane (unstable).
The $\mathbf{H}^2$ stability threshold is
$\xi^*(8) = 8 - \sqrt{63} \approx 0.063$.
With Jupiter's effective $\xi_{\mathrm{eff}} \sim e^{-r/L_R}
\sim e^{-3.3} \approx 0.037 < 0.063$,
the octagon is stabilized by the hyperbolic curvature of the
Rossby-deformed vorticity field---consistent with Juno observations.

\paragraph{Cosmological predictions.}
From the $N = 11$ energy budget and the radion potential (Paper~IV):
\begin{itemize}
\item \emph{EWPT gravitational waves} at $f \sim 3 \times 10^{-4}$\,Hz
  (LISA band), $\Omega_{\mathrm{GW}} h^2 \sim 10^{-11}$--$10^{-8}$
  depending on bubble wall velocity.
  Detectable by LISA ($\sigma \sim 10^{-13}$).
\item \emph{Neutrino mass sum} $\Sigma m_\nu = 0.059$\,eV,
  detectable by DESI $+$ CMB-S4 at $3\sigma$ sensitivity $\sim 0.02$\,eV.
\item \emph{Inflation} from the radion inflection point:
  $n_s = 0.966 \pm 0.003$, $r \sim 10^{-6}$ (undetectable),
  $dn_s/d\ln k \approx -6 \times 10^{-4}$.
\item \emph{Proton exactly stable} ($\mathbb{Z}_{56}$ charge conservation);
  testable at Hyper-Kamiokande ($\tau_p > 10^{35}$\,yr by ${\sim}\,2035$).
\item \emph{Dark matter invisible} to direct detection
  ($\sigma \sim 10^{-103}\;\mathrm{cm}^2$);
  detectable only via gravitational probes and structure formation.
\end{itemize}
...
quantized vortices with circulation
...
\item \emph{Triangular vortex lattice.}
  ...
  predicts that the vortex array forms a triangular lattice.
  The logarithmic vortex interaction operates with a neutralising
  background (the mean vorticity), placing it in the Abrikosov
  regime where the triangular lattice is the unique stable
  regular tiling.
...
\item \emph{Quantised glitch sizes.}
  ...
  \]
  is \emph{quantized} in units of $N = 3, 4, 5, 6, 7$.
...
\noindent
This is not a hypothetical application:
\citet{Howitt2020} directly cite Thomson~(1883) and
Havelock~(1931) for superfluid vortex array stability in
neutron stars, using the same $\csc^2$ kernel and $N \le 7$
stability bound.
The Havelock framework is already deployed in pulsar
glitch modelling.
...
sphere, potentially stabilizing polar polygons that would be
...
electron gas supports quantized magnetic flux tubes
