...
\begin{enumerate}
  \item \emph{Concrete test cases.}
    ...
    \code{4.38569.5t5.a.a}, \code{4.65657.5t5.a.a},
    \code{4.24217.5t5.a.a}
  ...
  \item \emph{The KK product at threshold.}
    ...
    ($\tau(P_-) = 0.644 \pm 0.010$ on the Bolza surface).
...
\paragraph{Gravitational Langlands (from Paper~III).}
The palindromic polynomials classified in Paper~I
(Proposition~\ref{I-prop:alg-int}) are quadratic, with
real-quadratic Galois group; their higher-degree
generalizations on arithmetic spatial slices with
trace field degree $d \ge 5$ are expected to have
non-solvable Galois groups (an open problem beyond the
scope of Paper~I).
Since the gravitational interaction (Paper~II)
...
The chain: $2{+}1$D AdS gravity with point masses on
$\Gamma\backslash\mathbf{H}^2$ $\to$ logarithmic Green's
function $\to$ palindromic thresholds $\to$ $S_5$ Galois
group $\to$ $\mathrm{GL}(4)$ Artin representations.
The Selberg zeta $Z_\Gamma(s)$ encodes both the gravitational
partition function ($Z_\Gamma'(1)$) and the stability
correction ($\delta C_1$) at $s = 1$; a Langlands lift
$\pi$ would constrain both simultaneously.
...
palindromic quartic, gives a complete characterization of
...
order~$48$; \citealt{Buser1992}) has systole trace
$B = 2 + 2\sqrt{2}$, which satisfies $u^2 - 4u - 4 = 0$.
By the palindromic construction of
Conjecture~\ref{I-conj:higher-genus}, the stability threshold
...
\]
of level $128$, LMFDB label \code{128.1.d.a},
...
\begin{remark}[The factorization bridge]
\label{rmk:factorization-bridge}
The sign of $a_c$ factorizes into a
...
\begin{theorem}[Modularity from the Sym$^2$ factorization]
...
\begin{proof}
\emph{Step 1 (Sym$^2$ factorization).}
...
The key new ingredient is Step~1: the Sym$^2$ factorization
...
$p \equiv 1 \pmod{4}$ \citep{Ribenboim1996}.
...
(Conjecture~\ref{I-conj:higher-genus}) is a functor to the
...
is the naturality constraint on the fiber functor
$\Phi \colon \mathbf{AHS} \to \mathbf{Sets}$
that sends each surface to its set of stable polygon sizes.
\end{remark}

\section{Non-solvable palindromic polynomials and the Langlands program}
...
(Hilbert reciprocity; \citealt{Voight2021}, \S14.6).
\end{proof}
...
\code{4.38569.5t5.a.a},
\code{4.65657.5t5.a.a}, and
\code{4.24217.5t5.a.a}
...
$\Delta^3$:
\code{4.57374000974009.10t12.a.a},
\code{4.283036930148393.10t12.a.a}, and
\code{4.14202376626313.10t12.a.a}.
