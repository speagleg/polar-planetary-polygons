...
the marginal eigenvalue to $+4\varepsilon^2$ at leading order.
For sufficiently small $\varepsilon$, this dominates both the
$O(\varepsilon^4)$ center-sector correction (next blob order) and
the $O(\varepsilon^6/d_{\min}^6)$ Schur coupling, giving
$\lambda_{\min} > 0$.  For the full range $\varepsilon < d_{\min}$:
direct computation of the constrained Hessian eigenvalue confirms
$\lambda_{\min}(\varepsilon) > 0$ at all tested values
$\varepsilon/d_{\min} \in \{0.01, 0.05, \ldots, 0.99\}$, with
minimum value $\lambda_{\min} \approx 0.30$ at $\varepsilon/d_{\min}
= 0.99$ (well above zero).  The $N = 7$ case is more delicate than
$N \le 6$: it relies on the $O(\varepsilon^2)$ blob lift rather
than a spectral gap, and the rigorous bound is
$\varepsilon/d_{\min} \ll 1$; the extension to $\varepsilon/d_{\min}
\lesssim 1$ is numerical.
\end{corollary}
