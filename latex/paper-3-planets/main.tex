...
\title{\seriestitle\\[6pt]\large Paper III: Planetary and Astrophysical Applications}
...
with the Juno data (see Section~\ref{II-sec:deformation} for
quantitative limits).
...
Section~\ref{II-sec:k0-interaction} (the $K_0$ stability table).
...
Proposition~\ref{II-prop:kappa-crit} can be compared with laboratory and
...
Proposition~\ref{II-prop:kappa-crit} for $N = 8$.
...
(Remark~\ref{I-rem:rg-fixed-point}; Corollary~\ref{I-cor:dilation-rg}
...
Theorem~\ref{II-thm:constrained-min} bridges this to the Onsager
...
from the unconstrained Onsager picture (Corollary~\ref{II-cor:min-not-max}).

\smallskip\noindent
What is new:
(1)~the constrained-minimum identification and its
Lagrange-multiplier derivation;
(2)~the analytic sign rule (Theorem~\ref{I-thm:sign-rule});
...
(Proposition~\ref{I-prop:rg-classification});
...
(Propositions~\ref{I-prop:curvature-dichotomy}--\ref{I-prop:morse-index});
...
(Section~\ref{I-sec:algebraic-structure});
...
(Theorem~\ref{II-thm:constrained-min}), the sign rule
(Theorem~\ref{I-thm:sign-rule}), and the stability table---are proven
...
vortex blob analysis (Section~\ref{II-sec:blob}) establishes that the
stability boundary is preserved under regularization and that the
$N = 7$ marginal case is resolved.

The Saturn bridge (Section~\ref{II-sec:rossby-bridge}) connects the
...
as the Havelock Hessian (Remark~\ref{II-rem:catseye}).  This is verified
...
(Remark~\ref{II-rem:catseye}), satisfying the conditions of
Proposition~\ref{II-prop:inertia}.
...
(Section~\ref{I-sec:curved}); and
(iii)~the critical-layer bridge (Remark~\ref{II-rem:catseye}), which
...
The constrained energy minimum (Theorem~\ref{II-thm:constrained-min}) is
