...
The angular zero-point energy of the critical pair
(computed by integrating $(1/2)\sqrt{\lambda}$ against
the ground-state density $|\psi_0|^2$) is effectively zero
(${\sim}10^{-27}$): the ground state sits in the classically
forbidden region where $\lambda < 0$ and the angular frequency
vanishes.  The $14\%$ baryon discrepancy---$4.22\%$ predicted
versus $4.93\%$ observed---is a genuine prediction of the
framework at $N = 11$, representing either a finite-$c$ correction
to the asymptotic mass gap $\sqrt{2/\pi}$ or a physical contribution
(e.g.\ from the neutrino sector) not yet included.
For a theory with \emph{zero} free parameters matching
the dark sector to sub-percent, a $14\%$ discrepancy in
the $5\%$ baryonic component is a
$0.7\%$ error on the total energy budget.
