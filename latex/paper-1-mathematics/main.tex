...
The universality follows from the proof structure of \S\ref{sec:classical-havelock}:
...
\emph{Proof sketch} (full derivation in companion Appendix~A).
Three contributions determine $C_1$:
(i)~the \emph{flat-plane Havelock term} $C_1^{(0)} = N{-}1$, which
gives the inter-vortex interaction in the $\xi \to 0$ limit;
(ii)~a \emph{curvature correction} arising from the difference
$h(d_{\mathbf{H}^2}) - h(d_{\mathrm{flat}})$ between the
hyperbolic and Euclidean pairwise interactions, evaluated at
the $N$-gon separations; and
(iii)~a \emph{Lagrange multiplier shift}: on $\mathbf{H}^2$ the
angular-impulse constraint involves the hyperbolic moment map
$L = \sum |z_k|^2/(1 - |z_k|^2/a^2)^2$, which changes $\mu_L$
relative to its flat value $-(N{-}1)/(4R^2)$.
The three combine into the rational function
$(N{-}1)(1{+}\xi^2)/(1{-}\xi)^2$, verified by explicit computation
of each term (Appendix~A, equations~A.1--A.5).
...
(code: \texttt{h2\_stability.py}) confirms this for $R/a \le 2$.
...
\begin{conjecture}[Index-theoretic origin of $N_{\mathrm{crit}} = 7$]
\label{conj:aps-index}
There exists a Dirac-type operator $D_N$ on the $\mathbb{Z}_N$-symmetric
sector of the vortex configuration space such that
$\mathrm{ind}(D_N) = \mu(N) = N - 5$ for $N \ge 8$, connecting the
stability boundary $N_{\mathrm{crit}} = 7$ to an
Atiyah--Patodi--Singer index theorem.  If true, this would give a
\emph{topological proof} that the flat-plane stability threshold
is~$7$: the number would follow from characteristic classes of the
configuration space rather than from the explicit eigenvalue formula.
\end{conjecture}

\noindent
By the APS theorem, $\mathrm{SF} = \mathrm{ind}(D_N)$ for
$D_N = \dd/\dd\xi + H_N(\xi)$.  This
equals $\mathrm{ind}(D_N)$ for $D_N = \dd/\dd\xi + H_N(\xi)$.
The conjecture asks whether $D_N$ has a geometric construction
independent of the explicit family $H_N(\xi)$---one that would
make the index computable from topology alone.
