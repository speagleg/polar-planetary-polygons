...
(Proposition~\ref{prop:palindromic}) is palindromic
...
\approx 0.16$, verified numerically.
...
\paragraph{Bolza verification.}
On the Bolza surface at $\xi_{\mathrm{Bolza}} = 0.217$
(where $N_{\mathrm{crit}} = 12$):
...
giving $\|h\|_{\ell^1} \le 12\,G(\ell_0) + O(e^{-2\ell_0})
\approx 0.02$ (direct computation from the Fuchsian generators;
the bound $|\delta C_1| \le 0.06$ from
Theorem~\ref{thm:index-pairing} is consistent).
At $N = 12$, $\xi = 0.217$: the binding eigenvalue is
$\min_m |c_m| \approx 0.79$.
The operator-norm gap ratio is $0.79/0.02 \approx 40$,
confirming the product identity with wide margin.
