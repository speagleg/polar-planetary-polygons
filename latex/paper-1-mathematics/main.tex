...
On compact quotients $\Gamma\backslash M$, the automorphic Green's function has an additional image sum, but this sum contributes a mode-independent correction $\delta C_1$ (by the Selberg trace formula), preserving the eigenvalue decomposition.
\end{remark}
...
Each trace $c$ gives a primitive hyperbolic element: $c = 3, 4, 6, 10, 16$ are not Chebyshev values $T_k(c')$ for any $k \ge 2$, $c' \ge 3$ (since $T_2(3) = 17$, $T_2(4) = 31$, etc.).
\end{proof}
