...
The CMS eigenstates at $\beta = 1$ are the Jack
polynomials $J_\lambda^{(1/\beta)} = J_\lambda^{(1)}$,
which are the Schur polynomials $s_\lambda$
--- the characters of $\mathrm{GL}(N)$.
...
\item \emph{Laughlin wavefunction (1983):}
  ...
  The Thomson stability boundary $N \le 7$ translates directly:
  the ``stable'' Laughlin fractions correspond to
  $\beta = 1, 3, 5, 7$ (odd, fermionic), and the $N = 7$
  Havelock boundary IS the $\nu = 1/7$ Laughlin state boundary.
...
\item \emph{Selberg integral (1944):}
  ...
  For $N = 2$ particles, the equilibrium positions
  are the $B_2$ zeros $(3 \pm \sqrt{3})/6$
