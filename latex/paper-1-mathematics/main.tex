...
(i)~$G_{\mathbf{H}^2}(z_0, \gamma z_0)
= O(e^{-\ell_\gamma})$ and the prime geodesic theorem give
$h_\xi^\Gamma \in \ell^1(\Gamma) \subset C^*_r(\Gamma)$;
Young's inequality gives~\eqref{eq:op-norm-bound}.
(ii)~The module $E = C^*_r(\Gamma)^{N-1}$ is free, so
$\mathcal{K}(E) = \mathcal{L}(E)$.
The gap condition ensures each $c_m + h$ is invertible,
making $F_\xi = \bigoplus_m \mathrm{sign}(c_m + h)$ a
well-defined self-adjoint unitary;
the Kasparov conditions hold automatically.
(iii)~Under the gap condition, the straight-line homotopy
$t \mapsto \mathrm{sign}(c_m + th)$ connects
$\mathrm{sign}(c_m + h)$ to $\mathrm{sign}(c_m) \cdot 1$
inside the invertibles of $C^*_r(\Gamma)$, so
$\tau_*(\alpha_N \otimes [D_S])
= \sum_m \mathrm{sign}(c_m)\,[\rho_m]
= [\mu_N(\mathbf{H}^2, \xi)]
= [\mu_N(\Gamma, \xi)]$
(the last step by Theorem~\ref{thm:spectral-flow}).
\end{proof}
