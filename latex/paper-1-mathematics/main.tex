...
$\ell_\gamma$ is the geodesic length, and $w_\gamma$ is a
weight depending on $\xi$ and the $N$-gon geometry.
\end{theorem}

\begin{proof}
The curvature coefficient on $S$ involves the Green's function
at the centre:
$C_1(S, \xi) = C_1(\mathbf{H}^2, \xi) + \delta C_1$
where $\delta C_1$ is the difference between the automorphic
Green's function $G_S(z_0, z_0)$ and its universal-cover
counterpart.  The automorphic Green's function has the spectral
expansion $G_S(z,w) = \sum_n \varphi_n(z)\overline{\varphi_n(w)}
/ \lambda_n$, so the diagonal restriction is the trace of
$\Delta_S^{-1}$ at $z_0$---precisely~$\operatorname{Tr}(f_\xi(\Delta_S))$.
The Selberg trace formula
(identity contribution $= C_1(\mathbf{H}^2,\xi)$;
hyperbolic conjugacy classes $= \delta C_1$)
gives~\eqref{eq:selberg-delta}.
\end{proof}
...
Every element of $K_0(C^*_r(\Gamma))$ is the image of a
$\Gamma$-equivariant $K$-homology class on $\underline{E}\Gamma
= \mathbf{H}^2$.
The Havelock test function $f_\xi$ defines such a class:
it is $\Gamma$-equivariant (since the Green's function is
$\Gamma$-invariant) and compactly supported in the spectral
variable (the sum~\eqref{eq:selberg-delta} converges absolutely,
as established by the operator-norm bound~\eqref{eq:op-norm-bound}).
