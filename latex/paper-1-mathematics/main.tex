...
\paragraph{Bolza verification.}
On the Bolza surface at
$\xi_{\mathrm{Bolza}} \approx 0.217$
(the smallest root of the palindromic quartic
in Conjecture~\ref{prop:higher-genus},
giving $N_{\mathrm{crit}} = 12$ at this threshold):
$|\delta C_1| \le 2/(4\pi\lambda_{\mathrm{triv}}) \approx 0.011$
and $\lambda_{\mathrm{bind}}(12,1) \approx 0.79$.
The bound $0.011 < 0.79$ is satisfied with a factor of $72$,
so the spectral flow on the Bolza surface equals
that on~$\mathbf{H}^2$.
The $71$-fold margin means that the spectral flow is preserved
on \emph{every} arithmetic surface whose systole exceeds
$\ell \ge 2\operatorname{arccosh}(1{+}(0.79/2)\cdot 4\pi)
\approx 4.1$ (a mild condition: the Bolza systole is~$3.06$,
but even surfaces with shorter systoles would need
$\delta C_1 > 0.79$ to change the spectral flow).
...
\paragraph{Bolza verification.}
On the Bolza surface ($g = 2$, systole $\ell_0 \approx 3.06$,
$12$ systole geodesics):
the $\ell^1$-norm is dominated by the $12$ shortest geodesics,
giving $\|h\|_{\ell^1} \le 12\,|G(\ell_0)|
+ O(e^{-2\ell_0}) \approx 0.18$
(using $|G_{\mathbf{H}^2}(d)| = (2\pi)^{-1}|\!\ln\tanh(d/2)|
\approx 0.015$ at $d = \ell_0$).
The spectral bound gives the tighter estimate
$|\delta C_1| \le 2/(4\pi\lambda_{\mathrm{triv}})
\approx 0.011$
(Theorem~\ref{thm:index-pairing}).
At $N = 12$, $\xi = 0.217$: the binding eigenvalue is
$\min_m |c_m| \approx 0.79$.
The gap ratio $0.79/0.011 \approx 72$ for the trace
and $0.79/0.18 \approx 4.4$ for the operator norm,
both confirming the product identity.
