...
perturbations): relevant perturbations destroy polygon stability.
...
model at coupling $\beta = 1$
\citep{Sutherland1971, CalogeroJMP1971}.
The Havelock eigenvalues $f(m,N) = m(N{-}m)/2$ are the CMS
single-particle excitation energies (identical at all~$\beta$;
they are the Haldane--Shastry dispersion relation).
At $\beta = 1$ the CMS interaction vanishes and the
eigenstates are the Jack polynomials
$J_\lambda^{(1/\beta)} = J_\lambda^{(1)} = s_\lambda$
(Schur polynomials, the characters of $\mathrm{GL}(N)$).
At general $\beta$, the Jack polynomials $J_\lambda^{(1/\beta)}$
are the eigenstates; the eigenvalues $f(m,N)$ are
$\beta$-independent.
...
is mode-dependent in isolation.
However, the trace constraint (Step~2) gives
$H_{rr}^{(m)} + H_{tt}^{(m)} = \delta$ (mode-independent).
Since $M_{p,rr}^{\mathrm{sing}} = -1/(2\pi d_p^2)$ (opposite sign
to $M_{p,tt}^{\mathrm{sing}}$), the singular parts of $H_{rr}^{(m)}$
and $H_{tt}^{(m)}$ are $-\alpha T_m$ and $+\alpha T_m$ respectively.
Therefore $H_{tt}^{(m)} = \alpha T_m + \tilde{C}_1$ where
$\tilde{C}_1 = \delta - \tilde{C}_1'$ absorbs all smooth-part
contributions, and the mode-dependent part is exactly
$\alpha\,T_m$.
...
with the $7 \to 8$ transition at $R/a \approx 0.52$.
...
\begin{proof}[Derivation of~$b(N)$]
The offset $b(N)$ is \emph{extracted} from the eigenvalue data:
$b(N) = C_1(\xi) - \log(2\sinh\rho)$ for $\xi = \tanh^2(\rho/2)$,
which gives the same numerical value at all~$\rho$ (verified
to $10^{-16}$ for $N = 3, \ldots, 24$).
The closed form decomposes as $b(N) = \langle f_m\rangle + \langle S_m\rangle$,
where $\langle f\rangle = N(N{+}1)/12$ is the mean Casimir and
$\langle S\rangle = \log N/(N{-}1) - \log 2$ is the mean log-sine sum.
The leading term $\langle f\rangle = N^2/12 + O(N)$ equals
$B_2 N^2/2$ (the Euler--Maclaurin correction to the discrete
log-sine kernel on $\mathbb{Z}/N\mathbb{Z}$, verified to
$R^2 = 0.9999999$).
Therefore
\[
  f_{\mathrm{crit}} - b(N)
...
\S\ref{sec:bolza-modular} and the palindromic growth
...
The Havelock Hessian on $\Gamma\backslash\mathbf{H}^2$ defines
an element of the Kasparov group
$KK(C^*(\mathbb{Z}_N),\, C^*_r(\Gamma))$, and the Kasparov
product with the fundamental class of the surface recovers the
Morse index (Theorem~\ref{thm:kk-product}).
Away from palindromic thresholds, this reduces to the scalar
$\delta C_1$ (\S\ref{sec:index-pairing}).
At the threshold, it does not:
the spectral projection becomes a non-trivial element of the
von~Neumann algebra $L(\Gamma)$, and the $L^2$-Morse index
is \emph{fractional}---on the Bolza surface, $61\%$ of the
$L^2$-weight is unstable while the automorphism-fixed centre
remains stable.
The stability of a vortex polygon depends on its
\emph{placement} on the surface, a phenomenon invisible to
the representation ring $R(\mathbb{Z}_N)$ and to the
trace-formula correction $\delta C_1$ alike.
...
The absolute convergence follows from the prime geodesic theorem
...
\begin{enumerate}
  ...
  \item[\emph{(c)}]
    ...
    applied to $[P_-]$ gives the $L^2$-Morse index, which the Selberg
    trace formula decomposes as:
    \[
      \tau(P_-) = \underbrace{C_1(\mathbf{H}^2,\xi)}_{\text{local index}}
      \;+\; \underbrace{\delta C_1(\Gamma,\xi)}_{\text{global correction}}.
    \]
    The local term is the ``identity contribution'' of the trace formula;
    the global term is the sum over nontrivial conjugacy classes.
\end{enumerate}
\end{theorem}
...
\item \emph{$[F, u_\gamma]$ is compact for all $\gamma \in \Gamma$}:
  ...
  A trace-class perturbation of a scalar gives a
  spectral projection that commutes with the group action
  up to trace-class error, verifying the Fredholm axiom.
...
(c)~The canonical trace $\tau$ on $C^*_r(\Gamma)$ is the coefficient
of the identity element.
For the spectral projection $P_-$ of $c_m + h$:
\[
  \tau(P_-) = \sum_{n \ge 0}
    \frac{|\varphi_n(z_0)|^2}{\mathrm{Area}(S)}\,
    \chi_{(-\infty,0)}(\lambda_n),
\]
where $\lambda_n$ are the eigenvalues of $\Delta_S$ and
$\chi_{(-\infty,0)}$ is the indicator function.
The Selberg trace formula decomposes this into the identity
orbital integral ($= C_1(\mathbf{H}^2, \xi)$, the universal-cover
contribution) and the sum over hyperbolic conjugacy classes
($= \delta C_1$), as computed in Theorem~\ref{thm:index-pairing}.
\end{proof}
...
\subsection{The KK-theory programme}
\label{sec:kk-programme}

Theorem~\ref{thm:spectral-flow} uses the \emph{trace}
$|\delta C_1| = |\tau_{z_0}(h_\xi^\Gamma)|$.
For the $KK$-level identity, the relevant quantity is the
...
\begin{enumerate}
  \item The element $h_\xi^\Gamma$ is well-defined in $C^*_r(\Gamma)$
    with operator norm bounded by the spectral zeta function:
    \begin{equation}\label{eq:op-norm-bound}
      \|h_\xi^\Gamma\|_{C^*_r}
      \;\le\; \max_{n \ge 1}\,\frac{|\varphi_n(z_0)|^2}{\lambda_n}
      \;\le\; \frac{1}{4\pi(g{-}1)\,\lambda_1(S)},
    \end{equation}
    where $\lambda_1(S)$ is the first nonzero eigenvalue of
    $\Delta_S$ and $4\pi(g{-}1) = \mathrm{Area}(S)$.
...
\begin{proof}
(i)~The automorphic Green's function
$G_S(z,w) = \sum_{n \ge 1} \varphi_n(z)\overline{\varphi_n(w)}/\lambda_n$
converges on compact~$S$.  The convolution operator $h$ maps
$\ell^2(\Gamma) \to \ell^2(\Gamma)$ with operator norm
$\|h\|_{C^*_r} \le \sup_{n \ge 1} |\varphi_n(z_0)|^2/\lambda_n$
(the spectral radius of the resolvent at~$z_0$).
At the automorphism-fixed point~$z_0$,
eigenfunctions in non-trivial representations of $\mathrm{Aut}(S)$
vanish: $\varphi_n(z_0) = 0$ for $\varphi_n \notin V_{\mathrm{triv}}$.
For trivial-representation eigenfunctions
$\varphi_n \in V_{\mathrm{triv}}$:
$|\varphi_n(z_0)|^2 \le \|\varphi_n\|_\infty^2
\le C(S)/\mathrm{Area}(S)$
(from the Sobolev embedding $H^1(S) \hookrightarrow L^\infty(S)$
on the compact surface, with $C(S) = O(1)$ depending on
the spectral gap and geometry).
On the Bolza surface with $|\mathrm{Aut}| = 48$:
$C \le 1$ (the large automorphism group forces
sup-norm $\le$ $L^2$-norm$/\sqrt{\mathrm{Area}}$
for the few trivial-representation eigenfunctions).
This gives~\eqref{eq:op-norm-bound}.

(ii)~The module $E = C^*_r(\Gamma)^{N-1}$ is finitely
generated and free, so $\mathcal{K}(E) = \mathcal{L}(E)$
(every adjointable operator is ``compact'').
The operator-norm gap ensures $c_m \cdot 1 + h_\xi^\Gamma$
is invertible in $C^*_r(\Gamma)$ for each~$m$, so
$F_\xi$ is a well-defined self-adjoint unitary.
The Kasparov conditions $F^2 - 1 \in \mathcal{K}(E)$ and
$[F, \pi(a)] \in \mathcal{K}(E)$ hold automatically.

(iii)~Under the gap condition, $\mathrm{sign}(c_m + h)$
is connected to $\mathrm{sign}(c_m) \cdot 1$ by the path
$t \mapsto \mathrm{sign}(c_m + th)$, $t \in [0,1]$
(which stays in the invertibles since
$\|th\| \le \|h\| < |c_m|$).
Thus $[\mathrm{sign}(c_m + h)] = [\mathrm{sign}(c_m)]$
in $K_0(C^*_r(\Gamma))$.
Applying $\tau_*$: $\tau(\mathrm{sign}(c_m)) = \mathrm{sign}(c_m) \in \{-1,+1\}$.
Summing over modes:
$\tau_*(\alpha_N \otimes [D_S])
= \sum_m \mathrm{sign}(c_m)\,[\rho_m]
= [\mu_N(\mathbf{H}^2, \xi)]$.
Since $[\mu_N(\Gamma,\xi)] = [\mu_N(\mathbf{H}^2,\xi)]$
under the gap condition (Theorem~\ref{thm:spectral-flow}),
~\eqref{eq:kasparov-product} follows.
\end{proof}

\paragraph{Bolza verification.}
On the Bolza surface ($g = 2$, $\lambda_1 \approx 3.839$):
$\|h\|_{C^*_r} \le 1/(4\pi\cdot 3.839) \approx 0.021$.
At $N = 12$, $\xi = 0.217$: the binding eigenvalue is
$\min_m |c_m| \approx 0.79$.
The operator-norm gap ratio is $0.79/0.021 \approx 38$,
confirming the product identity with wide margin.
For \emph{any} compact arithmetic surface with $\lambda_1(S) > 0.16$:
the gap condition holds for all $N \le 15$.
The Selberg $\tfrac{1}{4}$-conjecture
($\lambda_1 \ge 1/4$ for congruence subgroups) implies the gap for
all congruence surfaces at all $N \le 23$.
...
The computed value $0.641$ lies \emph{outside} this
discrete set, proving that $h$ has mixed-sign
eigenvalues within at least one Aut-isotypic component
...
Remark~\ref{rmk:bolza-physics}) because the
