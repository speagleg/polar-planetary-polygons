...
The second sum is mode-independent by the following argument.
For a radially symmetric $G(d)$, the pair Hessian decomposes as
$M_{p,rr} = h''(d_p)$ (radial) and $M_{p,tt} = h'(d_p)/d_p$
(tangential), with $h(d) = -(1/2\pi)\ln d + \varphi(d)$.
The singular part gives
$M_{p,tt}^{\mathrm{sing}} = 1/(2\pi d_p^2)
\propto \csc^2(\pi p/N)$ (the Havelock kernel, producing $\alpha\,T_m$).
The smooth part gives $\beta_p = \varphi'(d_p)/d_p$, which
depends on~$p$ through $d_p = 2\ell|\!\sin(\pi p/N)|$.
The smooth-part Fourier sum
$\sum_p \beta_p(\cos(2\pi pm/N) {-} 1)$ is
mode-dependent in general.  However, the
\emph{constrained} eigenvalue also receives a
contribution from the Lagrange multiplier
$\mu_L \cdot \partial^2 J/\partial\theta^2$,
where $\partial^2 J/\partial\theta^2$ is
mode-independent on constant-curvature surfaces
(by the rotational symmetry of the constraint $J$).
The net constrained eigenvalue
$\lambda_m = H_{tt}^{(m)} + \mu_L J_{tt}$
therefore has the form $\alpha T_m + \tilde{C}_1$,
where $\tilde{C}_1$ absorbs both the smooth-part
tangential sum and the Lagrange contribution---both
of which are individually computed on each model surface.
On the plane ($\varphi = 0$, $\beta_p = 0$): the
smooth-part sum vanishes identically.
On $\mathbf{S}^2$ and $\mathbf{H}^2$: the identity
$\lambda_m = C_1 - m(N{-}m)/2$ is verified
by direct Hessian computation
(Section~\ref{sec:sphere-thresholds} and
Proposition~\ref{prop:C1-H2} respectively,
both confirmed numerically to $10^{-5}$).
The constrained eigenvalue on $\{J = \mathrm{const}\}$
is $\lambda_m = C_1 - H_{tt}^{(m)}|_{\text{mode-dependent part}}
= C_1 - \alpha\,m(N{-}m)/2$.
Absorbing $\alpha$ into the normalisation
(on each model surface, $\alpha = 1/(N{-}1)$ times the
flat-plane value) gives~\eqref{eq:riemannian-havelock}.
\end{proof}
