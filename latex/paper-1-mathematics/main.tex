...
Section~\ref{III-sec:extensions} for a detailed assessment).  The
...
\begin{enumerate}
  ...
  \item \textbf{Meso scale: polygon arrangement.}  Given $N$ vortex
    ...
    surface (Theorem~\ref{II-thm:constrained-min}).  By the energy-Casimir
...
The meso-scale proof (Theorem~\ref{II-thm:constrained-min}) applies to
...
(Proposition~\ref{III-prop:complementary}).  Both mechanisms share the
...
(Section~\ref{III-sec:n-selection}).
...
\begin{itemize}
  \item The blob bridge
    ...
    Proposition~\ref{II-prop:blob} is the approach rate to the
    fixed point.
  \item The $K_0$ interaction
    (Section~\ref{II-sec:k0-interaction}):
...
\emph{constrained minimum} (Theorem~\ref{II-thm:constrained-min} below).
...
(Section~\ref{II-sec:n7-bifurcation}).
\end{remark}
...
Appendix~\ref{A-app:c1_h2} for the algebraic simplification), the
