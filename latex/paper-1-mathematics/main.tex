...
\textbf{the Selberg trace formula is the heat-kernel proof of
the index theorem} for the Laplacian on~$S$.
The ``identity contribution'' is the local index
(the Havelock eigenvalue on $\mathbf{H}^2$);
the ``geometric side'' is the global correction
(the topology of~$\Gamma$).
In the language of the Baum--Connes assembly map
$\mu \colon K_0^\Gamma(E\Gamma) \to K_0(C^*_r(\Gamma))$
(an isomorphism for all discrete subgroups of
$\mathrm{SL}(2,\mathbb{R})$, \citealt{Kasparov1988}):
\emph{the spectral correction $\delta C_1$ is the image of the
Havelock test function under the assembly map}.
...
$L(1, f) \approx 0.816$,
$L(1, f \otimes ({\cdot}/5)) \approx 1.056$,
$L(1, f \otimes \chi_4) \approx 1.138 - 0.104\,i$
