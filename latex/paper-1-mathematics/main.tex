...
$c = 2 + 16/(N{-}7)$.
The palindromic structure of the threshold equation generalises
to arbitrary genus, with the degree controlled by the trace field.
\end{abstract}
...
The RG terminology below is an analogy (the map $T_b$ is a pointwise rescaling, not a coarse-graining); the mathematical content is the stability classification itself.
...
\paragraph{The $\csc^2$ kernel as a universal object.}
...
Conditionally on the Selberg $\tfrac{1}{4}$-conjecture
($\lambda_1 \ge 1/4$ for congruence subgroups, unproved
in general but verified for small level),
