...
\textup{(a)}
The unconstrained Havelock eigenvalue $\lambda_m = (N{-}1) - m(N{-}m)/2$
is nonnegative for all $m$ if and only if $N \le 7$.
\textup{(b)}
The constrained Lagrangian eigenvalue
$\lambda_m^{\mathrm{constr}} = 3(N{-}1)/2 - m(N{-}m)/2$
is nonnegative for all $m$ if and only if $N \le 10$.
Thus the $N$-gon is an unconstrained saddle for $N \ge 8$
but a constrained minimum on $\{L = L_0\}$ for $N \le 10$.
The marginal case $N = 7$ (where $\lambda_3 = 0$) is resolved
