...
We prove the sign rule for general interactions, extend the
...
$c = 2 + 16/(N{-}7)$.
The palindromic structure of the threshold equation is
conjectured to generalise to arbitrary genus (Conjecture~\ref{prop:higher-genus}), with the degree controlled by the trace field.
\end{abstract}

\paragraph{Series overview.}
This is Paper~I of a four-paper series.
Paper~I develops the mathematics of the Havelock decomposition
$\lambda_m = C_1 - m(N{-}m)/2$: the sign rule, the Riemannian
extension, and the algebraic number theory of the
$\mathbf{H}^2$ thresholds.
Paper~II applies the decomposition to vortex stability
(the constrained energy minimum for $N \le 7$,
the blob bridge, and planetary/BEC predictions).
Paper~III connects the eigenvalue structure to gravity
(the CMS--CS Casimir identification, the graviton at $N = 7$,
and the Einstein equations from polygon entropy maximisation).
Paper~IV constructs the Standard Model gauge group and
cosmological parameters from the $N = 7$ and $N = 11$
polygon sectors on the Seifert manifold
$\mathbf{H}^2 \times_N S^1$.
Each paper is self-contained; the series builds cumulatively.
...
Paper~IV.  The constrained energy landscape and
...
for $N \le 7$---is proved in Paper~II (Theorem~\ref*{II-thm:constrained-min}), along with the
...
predictions) are developed in Paper~II.
...
\paragraph{Logical status convention (used throughout the series).}
Results are labeled \textbf{Theorem} (proved from stated hypotheses),
\textbf{Proposition} (proved, subsidiary),
\textbf{Identification} (physical correspondence, clearly marked),
or \textbf{Conjecture} (supported by evidence, not proved).
Papers~I and~II contain only Theorems and Propositions.
Papers~III and~IV introduce Identifications where the
vortex-spacetime correspondence is invoked.
...
This section establishes properties of the logarithmic interaction
...
The scale-classification terminology below is an analogy (the map $T_b$ is a pointwise rescaling, not a coarse-graining); the mathematical content is the stability classification itself.
...
The sign rule ($H''(0) < 0$) is an \emph{unconstrained} statement.
The physical dynamics constrains $L = \mathrm{const}$; the Lagrange
multiplier $\mu_L < 0$ shifts the curvature by $-2\mu_L > 0$,
converting the saddle into a constrained minimum for $N \le 7$
(Paper~II, Theorem~8.2).
\end{remark}
...
ring is in the stable zone (the planetary analysis is in Paper~IV;
...
$7 \to 8$   & $8 - 3\sqrt{7} = 0.0627\ldots$ & $0.51$ &
  8-gon stabilises \\
$8 \to 9$   & $5 - 2\sqrt{6} = 0.1010\ldots$ & $0.66$ &
  9-gon stabilises \\
...
\begin{enumerate}
  ...
  \item \emph{Non-constant curvature.}
    ...
    Paper~IV shows this effect is critical for Saturn's hexagon
