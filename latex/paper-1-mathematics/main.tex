...
\begin{abstract}
The stability eigenvalue of a regular $N$-gon of point vortices
decomposes as $\lambda_m = C_1(\text{surface}) - m(N{-}m)/2$,
separating a single surface-dependent number from a universal
Casimir determined by the cyclic group~$\mathbb{Z}_N$.
I prove the sign rule for general interactions, extend the
Havelock identity to constant-curvature Riemannian surfaces,
and show that the five algebraic-integer thresholds on the
hyperbolic plane are Pell units in quadratic fields (terminating
at~$\xi^*(23) = \varphi^{-2}$) and correspond to closed
geodesics on the modular surface with traces
$c = 2 + 16/(N{-}7)$.
\end{abstract}
...
consequences.  The present paper develops six distinct routes:
the first,  $m$-dependent part $f(m,N)$ is metric-independent
(Section~\ref{sec:havelock-identity});
second, the sign rule for $H''(0) < 0$ follows from the logarithm's
dilation invariance (Section~\ref{sec:sign-rule});
third, the stability threshold $N_{\mathrm{crit}} = 7$ is a closed-form
output of the $\mathbb{Z}_N$ Casimir
(Corollary~\ref{cor:why-7});
fourth, the curvature stability dichotomy
($K > 0$ destabilises, $K < 0$ stabilises)
is topological
(Proposition~\ref{prop:curvature-dichotomy}); fifth, 
...
and finally, the threshold polynomials on $\mathbf{H}^2$ connect to Pell
...
\begin{enumerate}
  ...
  \item \textbf{Algebraic structure of the stability boundary}
    ...
    (Conjecture~\ref{prop:higher-genus}),
...
$N$-body level.
This symmetry explains the \emph{scale-independence}
...
(\texttt{proofs/riemannian\_havelock\_step3.py}).
\end{proof}

\begin{remark}[Compact quotients]
On compact quotients $\Gamma\backslash M$, the automorphic Green's function has an additional image sum, but this sum contributes a mode-independent correction $\delta C_1$ (by the Selberg trace formula: the automorphic image sum $\sum_\gamma G(z, \gamma w)$ is $\mathbb{Z}_N$-invariant because the automorphic action commutes with the $\mathbb{Z}_N$ rotation, so its contribution to $C_1$ is mode-independent), preserving the eigenvalue decomposition.
\end{remark}
...
of the palindromic structure.
The threshold equation $\xi^2 - c\,\xi + 1 = 0$
(equation~\eqref{eq:palindromic}) is palindromic
because the Havelock Hessian is invariant under
conformal inversion $\xi \leftrightarrow 1/\xi$.
...
Each trace $c$ gives a primitive hyperbolic element: $T_2(x) = 2x^2{-}1$ gives $T_2^{-1}(c) = \sqrt{(c{+}1)/2}$, which is irrational for all $c \in \{3, 4, 6, 10, 16\}$ (none of $(c{+}1)/2$ is a perfect square). No $T_k$ for $k \ge 3$ yields these values ($T_3(2) = 26 > 16$). All five are primitive.
\end{proof}
...
The staircase $N_{\mathrm{crit}}(\xi) = 7, 8, 9, \ldots$
as $\xi$ increases through the palindromic thresholds is the
vortex analogue of the quantum Hall staircase
$\sigma_{xy} = 1, 2, 3, \ldots$ as $1/B$ increases through
...
\end{tabular}
\end{center}
In the IQHE, the transitions occur at equally spaced $1/B$
values (arithmetic progression); in the vortex staircase,
they occur at palindromic algebraic numbers (Pell units).
The algebraic number field of each step is an invariant of
the phase transition that has no IQHE analogue.
...
%% ====================================================================
\section{The $B_2$ organising principle}
\label{sec:b2-tower}

The three-layer decomposition reduces the Havelock eigenvalue
to a single Bernoulli number.
The asymptotic mode average of the $\mathbb{Z}_N$ orbifold
Casimir is $\langle m(N{-}m)\rangle/N^2 \to B_2 = 1/6$,
governing the growth law
$\rho^* \sim N^2/24$ (\S\ref{sec:algebraic-structure}).
...
and the Casimir vacuum energy is $\zeta(-1) = -B_2/2 = -1/12$:
the Dedekind eta exponent $q^{1/24}$, the Euler--Maclaurin
correction, and the vortex self-energy all arise from the
same $B_2 = 1/6$.

\paragraph{The stability polynomial.}
The polynomial $6B_2(x) = 6x^2 - 6x + 1$ is the shifted Legendre
polynomial $P_2(2x{-}1)$.
Its roots $(3 \pm \sqrt{3})/6$ are the two-point
\emph{Gauss--Legendre quadrature nodes} on~$[0,1]$---the
optimal sampling points for numerical integration.
In the vortex system, these roots give the normalised
critical mode $m^*/N$ at the stability threshold:
the polygon ``samples'' the $\csc^2$ interaction kernel at
the points that minimise the integration error of the
angular Casimir.
The palindromic symmetry $m \leftrightarrow N{-}m$ corresponds
to the Gauss node symmetry $x \leftrightarrow 1{-}x$ about
the midpoint.
The root product $(3{-}\sqrt{3})(3{+}\sqrt{3})/36 = 1/6 = B_2$:
the Bernoulli number is the product of its own zeros.

The Gauss--Legendre and Euler--Maclaurin formulae are dual:
the former converts integrals to sums (optimal interior nodes),
the latter converts sums to integrals (boundary corrections
$\propto B_{2k}$).
The vortex stability analysis is an Euler--Maclaurin
problem (the discrete Havelock sum $\sum\csc^2(\pi p/N)$
approximating the integral); the Gauss--Legendre
quadrature gives the optimal integrator for the same
Hamiltonian flow.
The $B_2$ zeros are where both operations agree
exactly---the stability boundary \emph{is} the
discrete--continuous meeting point.
The Casimir vacuum energy $\zeta(-1) = -B_2/2 = -1/12$
is the same $B_2$ in its regularisation role:
the csc$^2$ Laurent constant $2B_2 = 1/3$ governs
the self-interaction in both contexts.

\paragraph{The $\csc^2$ kernel in seven contexts.}
\label{rmk:csc2-universality}
The Havelock identity
$\sum_{p=1}^{N-1}\frac{1 - \cos(2\pi pm/N)}{4\sin^2(\pi p/N)} = \frac{m(N{-}m)}{2}$
--- proved for vortex stability (1931) ---
is the eigenvalue equation of the \emph{same} operator that
appears in several other contexts (each identification
below is established in the cited literature; we make
no new claims here):
\begin{enumerate}
\item \emph{Calogero--Moser--Sutherland (1969--71):}
  $H_{\mathrm{CMS}} = -\sum\partial_\theta^2
  + \beta(\beta{-}1)\sum_{j<k}\csc^2((\theta_j{-}\theta_k)/2)$.
  The vortex system is the classical ($\beta \to \infty$) limit.
\item \emph{Dyson circular ensemble (1962):}
  the equilibrium measure of $P(\theta) \propto
  \prod|e^{i\theta_j}{-}e^{i\theta_k}|^\beta$ at $\beta = 2$
  is the CMS ground state.
  The vortex $N$-gon is the zero-temperature limit.
\item \emph{Haldane--Shastry spin chain (1988):}
  the Polychronakos freezing trick freezes
  CMS particles to the Thomson ring ($\beta \to \infty$);
  the surviving spin excitations obey the HS Hamiltonian
  $H_{\mathrm{HS}} = \sum_{j<k}\csc^2(\pi(j{-}k)/N)\,
  (\mathbf{S}_j \cdot \mathbf{S}_k - 1/4)$,
  with spinon dispersion $\varepsilon_m = m(N{-}m)/2 = f(m,N)$
  (the Havelock Casimir).
\item \emph{Laughlin wavefunction (1983):}
  the edge theory of $\Psi_L = \prod(z_j{-}z_k)^m\,e^{-|z|^2/4}$
  reduces to the CMS model on the boundary circle
  with $\beta = m$ (the inverse filling fraction $1/\nu$).
  The CMS coupling $\beta$ equals the Laughlin filling
  denominator $m = 1/\nu$.
  The Havelock stability bound ($N \le 7$ vortices) constrains
  the \emph{ring size} at each~$\beta$; the Laughlin
  fractions $\nu = 1/m$ for odd~$m$ exist for all~$m$,
  but the edge excitations of the $\nu = 1/m$ state
  share the $\csc^2$ kernel at coupling $\beta = m$.
  At $m = 1$ (filled Landau level), the modes obey
  Haldane exclusion statistics with $g_s = 1$
  (fermionic): modes fill from $m = 1$ upward
  until the Fermi level reaches the instability boundary.
\item \emph{Selberg integral (1944):}
  the Selberg integral
  $S_N(\alpha) = \int_0^1 \prod|x_i{-}x_j|^{2\alpha}
  \prod x_i^{a-1}(1{-}x_i)^{b-1}\,dx$
  is the partition function of the CMS model on~$[0,1]$.
  For $N = 2$ particles in the symmetric case ($a = b = 1$),
  the equilibrium positions are the $B_2$ zeros
  $(3 \pm \sqrt{3})/6$
  (the Gauss--Legendre nodes).
\item \emph{$W_N$ algebra (1985--):}
  the CMS Hamiltonian is the $L_0$ mode of the Virasoro
  subalgebra of $W_N$; Jack polynomials at $\alpha = 1/\beta$
  are singular vectors.
\item \emph{AGT/Nekrasov (2009):}
  the Nekrasov partition function $Z(\varepsilon_1,\varepsilon_2)$
  equals the Virasoro conformal block via AGT, with
  $\varepsilon_1/\varepsilon_2 = \beta$.
\end{enumerate}
The Havelock identity is the classical limit of CMS,
the zero-temperature limit of Dyson,
the frozen-particle limit of Haldane--Shastry,
the edge limit of Laughlin,
the $N = 2$ equilibrium of the Selberg integral,
the Virasoro truncation of $W_N$,
and the $\varepsilon_2 \to 0$ limit of Nekrasov.
In each case, $B_2 = 1/6$ governs the self-interaction
correction ($\csc^2(x) = 1/x^2 + 2B_2 + O(x^2)$).

The $B_2$ circle closes through the index theorem:
the Haldane fractional exclusion statistics of the
CMS model at $g_s = 1$ (fermionic) are governed by the
Riemann--Roch theorem on the Landau level
\citep{LiOuvry1994},
whose leading correction is the Todd class $\mathrm{Td}
= 1 + c_1/2 + (c_1^2 + c_2)/12 + \cdots$,
where the coefficient $1/12 = B_2/2$ is the same
$B_2$ that appears in the Havelock offset $b(N)$
(Paper~III, Proposition~\ref{III-prop:equiv-rr}).
The chain CMS $\to$ Haldane statistics $\to$
Riemann--Roch $\to$ Todd class $\to$ $B_2 = 1/6$ $\to$
Havelock eigenvalue is therefore a closed loop:
the index theorem and the integrable system
see the same $B_2$ because they solve the same
spectral problem on the $\mathbb{Z}_N$ orbifold.

\paragraph{The $4/3$ self-interaction identity.}
The identity $4/3 = 1 + 2B_2$ connects two classical
self-interaction anomalies to the vortex system:
(i)~the \emph{vortex} ratio of the $\csc^2$ Laurent constant
to the small-ring curvature coefficient,
$(1/3)/(1/4) = 4/3$ (Paper~III, \S\ref{III-sec:lichnerowicz}, Step~4);
(ii)~the \emph{classical electron} $4/3$ problem
($\int_0^\pi \sin^3\!\theta\,d\theta = 4/3$: the angular
integral producing the electromagnetic momentum excess
in the Abraham--Lorentz model).
In both cases, the ``$1$'' is the singular (Coulomb/Casimir)
part of the kernel, and the ``$1/3 = 2B_2$'' is the
finite self-interaction remainder from the regular part.
The vortex system is distinguished by an \emph{exact} balance:
$\sum_{m=0}^{N-1}\lambda_m = 0$ for the circulant
(the diagonal of the interaction matrix vanishes), so no
regularisation or Poincar\'e stress is needed.
...
The stability results of the preceding sections treat each
arithmetic surface as a black box that contributes a single
number~$\delta C_1$.
This section opens the box, in three stages of increasing depth:
\begin{enumerate}
\item[\emph{(i)}]
  The \emph{elementary invariant}
  $[\mu_N(\xi)] \in R(\mathbb{Z}_N) \cong \mathbb{Z}^N$
  records the signs of the eigenvalues---a $K_0$-class that
  changes by $[\rho_m]{+}[\rho_{N-m}]$ at each palindromic
  threshold (\S\ref{sec:index-pairing}).
\item[\emph{(ii)}]
  On arithmetic quotients $\Gamma\backslash\mathbf{H}^2$,
  the Selberg trace formula computes $\delta C_1$ as a sum over
  closed geodesics, and spectral flow preservation
  (\S\ref{sec:spectral-flow}) shows the wall-crossing pattern
  is the \emph{same} as on~$\mathbf{H}^2$.
\item[\emph{(iii)}]
  At a palindromic threshold ($c_m = 0$), the scalar $\delta C_1$
  no longer determines the stability: the spectral projection
  becomes a non-trivial element of the von~Neumann algebra
  $L(\Gamma)$, and the $L^2$-Morse index is \emph{fractional}%
  ---on the Bolza surface, $\approx\!64\%$ of the $L^2$-weight
  is unstable while the automorphism-fixed centre remains stable
  (\S\ref{sec:gap-free}).
  The stability of a vortex polygon depends on its
  \emph{placement} on the surface, a phenomenon invisible to
  $R(\mathbb{Z}_N)$ and to $\delta C_1$ alike.
\end{enumerate}
The natural language for stage~(iii) is $KK$-theory:
the Havelock Hessian defines an element of the Kasparov group
$KK(C^*(\mathbb{Z}_N),\, C^*_r(\Gamma))$, and the Kasparov
product with the fundamental class of~$S$ recovers the Morse
index (\S\ref{sec:kk-programme}).
Away from thresholds, the Kasparov product reduces to the
scalar $\delta C_1$; at the threshold, it carries genuinely
new information.
...
\paragraph{Bolza verification.}
The Bolza bounds from the preceding paragraph
($|\delta C_1| \le 0.011$, binding eigenvalue $0.79$,
gap ratio~$72$) apply here: at $N = 12$,
...
\paragraph{Bolza verification.}
On the Bolza surface (same bounds as above:
$|\delta C_1| \le 0.011$, binding~$0.79$, gap ratio~$72$),
the $\ell^1$-norm gives a weaker but independent check:
$\|h\|_{\ell^1} \le 12|G(\ell_0)| \approx 0.18$
(gap ratio $0.79/0.18 \approx 4.4$), confirming the
product identity.
More generally, for any compact arithmetic surface whose
...
\begin{remark}[Topological phases]
...
The structure parallels the IQHE
(Bellissard): Chern number $\leftrightarrow$ Morse index,
gap closing $\leftrightarrow$ palindromic threshold,
magnetic field $\leftrightarrow$ curvature~$\xi$.
The vortex classification is richer ($\mathbb{Z}^N$ vs
$\mathbb{Z}$) because the palindromic algebra determines
\emph{which} mode crosses in \emph{which} number field.
