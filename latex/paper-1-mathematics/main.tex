...
The second sum is mode-independent because
$\sum_p \beta_p\,\cos(2\pi pm/N)$
is constant in~$m$ when $\beta_p$ is smooth and $O(1)$
(the Fourier coefficients of a smooth function on
$\mathbb{Z}/N\mathbb{Z}$ decay, but the leading-order term
$\sum \beta_p(\cos-1)$ is independent of~$m$ by the
same orthogonality as in Step~2).
...
\begin{proof}
(i)~Since $\delta C_1$ is mode-independent
(Section~\ref{sec:havelock-identity}), the eigenvalue on~$S$
is $\lambda_m^S(\xi) = \lambda_m^{\mathbf{H}^2}(\xi) + \delta C_1(\Gamma, \xi)$.
...
By the $L^2$-normalization
$\|\varphi_n\|_{L^2(S)}^2 = 1$ and $\mathrm{Area}(S) = 4\pi(g{-}1)$:
$|\varphi_n(z_0)|^2 \le 1/\mathrm{Area}(S)$,
giving~\eqref{eq:op-norm-bound}.
