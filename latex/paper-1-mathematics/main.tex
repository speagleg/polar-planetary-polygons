...
\begin{remark}[Topological phases and the integer quantum Hall analogy]
\label{rem:topological-phases}
The palindromic thresholds $\xi^*(N,m)$ are
\emph{critical values of the SO$(2)$ moment map} on the
$N$-vortex phase space: the reduced space
$\{L = \ell\}/\mathrm{SO}(2)$ changes topology at each
$\ell$ corresponding to $\xi^*(N,m)$, because a degenerate
critical point ($\lambda_m = 0$) appears in the energy
landscape.
The $K_0$ class $[\mu_N(\xi)]$ changes by
$[\rho_m] + [\rho_{N-m}]$ at each crossing---a
wall-crossing in the $K$-theory of the vortex Hamiltonian.

The structure parallels Bellissard's $K$-theoretic
classification of the integer quantum Hall effect:
\begin{center}
\small
\begin{tabular}{ll}
\toprule
...
Filled Landau levels &
  Stable $\mathbb{Z}_N$ modes \\
Chern number $n \in \mathbb{Z}$ &
  Morse index $[\mu_N] \in R(\mathbb{Z}_N) \cong
  \mathbb{Z}^N$ \\
Gap closing (level crossing) &
  Palindromic threshold $\xi^*(N,m)$ \\
Magnetic field $B$ &
  Curvature parameter $\xi$ \\
$\sigma_{xy} = n e^2/h$ &
  $N_{\mathrm{crit}}(\xi) = \max$ stable polygon \\
\bottomrule
\end{tabular}
\end{center}
The vortex classification is \emph{richer}:
$R(\mathbb{Z}_N) \cong \mathbb{Z}^N$ (one component per
Fourier mode) versus $\mathbb{Z}$ (one Chern number).
And the palindromic algebra determines \emph{which} mode
crosses zero and in \emph{which} algebraic number field the
crossing occurs---structure absent from the IQHE.
Under geometric quantisation of the $N$-vortex phase space
(a K\"ahler manifold), the palindromic thresholds become
gap-closing points of the quantum Hamiltonian, the Morse
index becomes the number of negative-energy quantum states,
and the $K_0$ class becomes a genuine Chern number---literal
topological phase transitions in the Kitaev~(2009) /
Freed--Hopkins~(2021) classification.

\paragraph{The stability staircase.}
The Morse index $\mu(N, \xi)$ as a function of curvature
is a \emph{staircase}: it decreases by~$2$ at each
palindromic threshold $\xi^*(N,m)$ (the conjugate pair
$\rho_m + \rho_{N-m}$ stabilises), except for the
self-conjugate mode $m = N/2$ (even~$N$) which contributes
$\Delta\mu = 1$.
The total staircase height is $\mu(N, 0) = N{-}5$, and
the spectral flow $\mathrm{SF}(H_N,\, 0 \to 1) = N{-}5$
(Theorem~\ref{thm:spectral-flow}).
For example ($N = 12$):
\begin{center}
\small
\begin{tabular}{ccccl}
\toprule
$\xi^*$ & modes & $\Delta\mu$ & $\mu$ & Field \\
\midrule
$0$ & --- & --- & $7$ & --- \\
$0.093$ & $3, 9$ & $-2$ & $5$ & --- \\
$0.160$ & $4, 8$ & $-2$ & $3$ & --- \\
$0.193$ & $5, 7$ & $-2$ & $1$ & --- \\
$0.202$ & $6, 6$ & $-1$ & $0$ & $\mathbb{Q}(\sqrt{11})$ \\
\bottomrule
\end{tabular}
\end{center}
The \emph{universal quantum} is $\Delta\mu = 2$
(one conjugate pair per step), analogous to
$\Delta\sigma_{xy} = e^2/h$ in the IQHE\@.
The physical observable that changes depends on the
realisation:
instability growth rate (classical vortices),
polygon lifetime (BEC, Paper~II \S\ref{II-sec:bec}),
shell transition (superconducting disks, Paper~II
Remark~\ref{II-rmk:superconductor}),
or gravitational decay channel (2+1D AdS, Paper~II
Proposition~\ref{II-prop:grav-ncrit}).
\end{remark}
