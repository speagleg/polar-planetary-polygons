...
\emph{Step 2}: since the off-diagonal blocks are proportional
to~$I_2$, the $xx$ and $yy$ Fourier components are equal and the
$xy$ component vanishes; each $2 \times 2$ Fourier block is scalar.
The isotropic Fourier block for mode $m \ge 2$ is therefore
$\sum_{p=1}^{N-1} 2(e^{-2\pi i pm/N}-1) = -2N$ (each of $xx$,
$yy$), and
$\delta\Omega = (1/R)\sum_{p=1}^{N-1} g'(d_p)(x_0{-}x_p)/d_p
= 2\sum (x_0{-}x_p) = 2N$.
Since $\delta\lambda_m = -\delta\Omega - v_m^\top\delta\nabla^2 H\,v_m
= -2N - (-2N) = 0$, the eigenvalue is unchanged.
...
$B = 2 + 2\sqrt{2}$, giving the palindromic quartic
...
\begin{proof}
A hyperbolic element $\gamma \in \mathrm{SL}(2,\mathbb{Z})$ has
a conjugate in $\Gamma_0(p)$ (i.e., a representative with
$p \mid c$ in $\gamma = \bigl[\begin{smallmatrix}a&b\\c&d
\end{smallmatrix}\bigr]$) if and only if the binary quadratic form
$Q(x,y) = cx^2 + (d{-}a)xy - by^2$ of discriminant $\Delta = B^2-4$
represents~$0$ modulo~$p$.  By the theory of quadratic forms
this holds iff $\Delta$ is a quadratic residue modulo $p$
(or $p \mid \Delta$), i.e., $(\Delta/p) \ne -1$.
The split/ramified/inert trichotomy corresponds to
$(\Delta/p) = +1$, $0$, $-1$ respectively, with the stated
multiplicities.
The table entries are computed from the Legendre symbol
for $\Delta \in \{5, 12, 32, 96, 252\}$ at each $p$.
By the Chebotarev density theorem, each $\Delta$ is a QR for
asymptotically half of all primes; the probability that all
five are simultaneously QR is $\sim 1/32$, predicting the
first ``fully visible'' prime near $p \approx 300$--$400$.
\end{proof}
