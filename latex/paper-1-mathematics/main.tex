...
On the Bolza surface: $\tau(P_-) = 0.644 \pm 0.001$
...
The Selberg $\tfrac{1}{4}$-conjecture
($\lambda_1 \ge 1/4$ for congruence subgroups) strengthens
this: systoles of congruence surfaces grow logarithmically
...
An analytic error bound follows from the spectral gap
$\lambda_1 \approx 3.839$ of the Bolza Laplacian
\citep{Buser1992}: the automorphic Green's function
coefficients at word length~$L$ decay as
$O(e^{-sL})$ with $s = \sqrt{\lambda_1 - 1/4} \approx 1.89$,
so the truncation error at $L = 4$ is
$O(e^{-5s}) \approx 8 \times 10^{-5}$.
The error propagation from the operator truncation
$\delta h$ to the spectral projection $\tau(P_-)$
is amplified by the near-zero spectral density:
from the spectrum, at most $15$ eigenvalues lie
within $O(e^{-5s})$ of zero, giving
$|\delta\tau(P_-)| \le 15 \times 8 \times 10^{-5}
\approx 0.001$.
Therefore $\tau(P_-) = 0.644 \pm 0.001$.
...
$1,1,2,2,2,3,3,4$; the achievable values near $0.6$
are $28/48$ and $30/48 = 5/8$
(with $29/48$ provably non-achievable).
The computed value $0.644 \pm 0.001$ lies \emph{outside} this
discrete set (the nearest achievable values are $28/48 \approx 0.583$
and $30/48 = 0.625$, both outside the error bar), showing that
$h$ has mixed-sign eigenvalues within at least one Aut-isotypic
component at the palindromic threshold.
