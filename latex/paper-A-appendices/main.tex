...
\item $C_1$ decreases monotonically from $N-1$ at the pole ($\xi = 0$)
  to $(N-1)/2$ as $\xi \to \infty$.  At the equator
  ($\xi = 1$): $C_1 = (N-1)/2$.  This is opposite to
  $\mathbf{H}^2$ where $C_1$ increases with~$\xi$.
...
\item Explicit thresholds:
  ...
  $\xi^*(5) = 3 - 2\sqrt{2}$,\;
  $\xi^*(6) = 9 - 4\sqrt{5}$.
  These are fundamental units (Pell solutions) in the quadratic
  fields $\mathbb{Q}$, $\mathbb{Q}(\sqrt{3})$,
  $\mathbb{Q}(\sqrt{2})$, $\mathbb{Q}(\sqrt{5})$---the same
  algebraic structure as the $\mathbf{H}^2$ thresholds.
