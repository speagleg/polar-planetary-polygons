...
In $d \ge 3$, condition~(H1) provides a new
characterisation of Einstein manifolds.
\end{abstract}
...
The series rests on one physical hypothesis:
\emph{point-vortex polygon dynamics on~$\mathbf{H}^2$
is spacetime dynamics} --- specifically, the Regge vertex
(deficit angle $\varepsilon_v$) is identified with a
point vortex (circulation $\kappa = \varepsilon_v/(2\pi)$).
This is a kinematic identity (both are logarithmic
sources of the 2D Green's function), but the claim that
the Onsager selection principle on this system selects
the physical spacetime geometry is the essential
\emph{physical postulate}, not a theorem.
The mathematical content of conditions (H1)--(H3) is
proved; their physical application to gravity via this
postulate is the framework's testable hypothesis.
...
As $\varepsilon \to 0$, the maximiser converges by
$\Gamma$-convergence (\citealt{DalMaso1993}, Prop.~6.8):
uniform convergence of integrands (Hadamard),
compactness of $\{g : \mathrm{Area} = A\}$, and
strict concavity upgrade subsequential to full convergence.
The self-referential metric dependence of~$G_S$ enters at
$O(\varepsilon^2)$ and does not affect the maximiser,
since $R = \mathrm{const}$ holds at every order.
...
In $d_{\mathrm{st}} \ge 4$, the $\lfloor N/2\rfloor$
independent angular probes of~(H1) constrain the traceless
Ricci tensor; the full spacetime equations require additionally
the boundary Virasoro mechanism (Paper~IV).
\end{proof}
...
$\gamma \sim O(N)$.
It is uniquely fixed: any other value would break
mode-independence of~$\Phi$ (the defining property of
