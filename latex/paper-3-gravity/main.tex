...
\noindent
\emph{Strict concavity: non-perturbative argument.}
...
The coincident Green's function
$G_\varepsilon(x,x)$ encodes the local geometry at
scale~$\varepsilon$; constancy of
$C_1(x) = G_\varepsilon(x,x) + b(N)$ implies
isotropy of the metric by Schur's lemma,
hence $R = \mathrm{const}$.
This argument holds at every~$\varepsilon$
with no Seeley--DeWitt expansion.
...
The second angular derivative of this kernel is
$\csc^2(\pi p/N)/(4\sin^2(\pi p/N))$,
so the $m$-th $\mathbb{Z}_N$ Fourier eigenvalue of
the Hessian is
$\sum_{p=1}^{N-1}\csc^2(\pi p/N)
\bigl(1 - \cos(2\pi pm/N)\bigr)/2 = m(N{-}m)/2$
