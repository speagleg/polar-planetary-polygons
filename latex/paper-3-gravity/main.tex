...
both hypotheses are derived (H1~from the $\csc^2$
uniqueness theorem, H3~from the Hadamard universality
of the Green's function).
The cosmological constant~$\Lambda$ is determined separately
by the WDW equation (Proposition~\ref{prop:wdw-lambda}).
...
the relevant topological constraint is weaker than for compact
groups \citep[\S3]{Witten1988}.
...
\item \emph{OPE coefficient bound (Liouville action):}
  ...
  $C_k^2 \lesssim \exp(-N^2\ln N/12)$---doubly exponential
  suppression.
  The total non-identity contribution is bounded by
  $(N{-}1)\exp(-b(N)\ln N) \le 0.1\%$ for $N \ge 7$
...
$C_1(\xi)$ as in~\eqref{eq:C1-H2}, where
$\xi = \tanh^2(\rho/2)$.
...
Einstein equations.
In $d = 2$ ($2{+}1$D), the two hypotheses below are
...
spin-$2$ graviton of Theorem~\ref*{V-thm:graviton-4d}).
...
\item[\textup{(H3)}]
  ...
  at every point~$x \in S$ and for all sufficiently
  small~$\varepsilon > 0$.
  ...
  at every~$x$ and every~$\varepsilon$ in the Hadamard regime.
\end{enumerate}
...
By~(H3), the expansion~(\ref{eq:C1-expansion-proof}) holds
at every~$x$ and for every sufficiently small~$\varepsilon$.
The $O(\varepsilon^0)$ coefficient $(N{-}1)$ is $x$-independent
(it is the flat-plane limit).
The $O(\varepsilon^2)$ coefficient $-(N{-}1)R(x)/4$ must also be
$x$-independent, since $C_1(x,\varepsilon)$ is determined
by the Green's function of $(S,g)$, whose mode-independent
structure~(H1) constrains the $\varepsilon$-expansion at
every order.
Therefore $R(x) = R_0$ for all~$x$, with
$R_0 = -4[f(m^*,N) - (N{-}1)]/[(N{-}1)\varepsilon^2]$
at the threshold~$\varepsilon$ where $C_1 = f(m^*,N)$.
...
\textup{(H1)} holds at every point (which is automatic
...
\label{rmk:higher-d}
Steps~1--3 are dimension-independent: linearised gravity
...
The angular bath consists of free harmonic oscillators
(the Havelock modes) living on the boundary circle~$S^1$.
At each palindromic threshold, the critical eigenvalue
$\lambda_{m^*} = 0$ creates a \emph{spatial bipartition}
of the boundary circle: the Havelock modes with $m < m^*$
occupy one arc and those with $m > m^*$ the complementary arc
(the Frobenius orbits $O_+$ and $O_-$ in the $\mathbb{Z}_N$
decomposition).
By the Bisognano--Wichmann theorem for free fields on $S^1$,
the modular Hamiltonian of the vacuum state restricted to one
arc equals the Rindler boost Hamiltonian on that arc, with
Unruh temperature determined by the surface gravity at
the entangling point.
