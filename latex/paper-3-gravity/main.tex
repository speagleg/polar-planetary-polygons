...
[K(x,x;t) - 1/A]\,dt$),
the identity acquires an $O(\varepsilon^2\,\Delta K)$
correction; elliptic bootstrapping
($K = \bar{K} + O(\varepsilon^2) \Rightarrow
\Delta K = O(\varepsilon^2) \Rightarrow
K = \bar{K} + O(\varepsilon^4) \Rightarrow \cdots$)
closes the iteration at $K = \bar{K}$ exactly.
...
\item \emph{Geometric origin (Fisher--Rao).}
  ...
  The constant~$c$ in the FK Hamiltonian
  $H = -(1/2c)\,d^2/d\rho^2 + V(\rho)$
  is determined by the eigenvalue decomposition:
  $c = 12\,b(N)$ is the unique value at which
  the mode-independent offset~$b(N)$ in
  $\lambda_m = C_1(\rho) - f(m,N)$ absorbs all
  $\rho$-dependence into a single function~$C_1$,
  giving a $\rho$-independent kinetic coefficient
  (step~3).

\item \emph{Exact coefficient (overdetermined consistency).}
  Three independent calculations determine~$c$:
  \begin{itemize}
  \item[\emph{(a)}] \emph{Eigenvalue arithmetic.}
    $b(N)$ is the unique mode-independent offset in
    $\lambda_m = C_1(\rho) - f(m,N)$
    (Proposition~\ref{prop:equiv-rr}).
    Setting $c_1 = 12\,b(N)$ is a definition.
  \item[\emph{(b)}] \emph{Chern--Simons topology.}
  ...
  \item[\emph{(c)}] \emph{Boundary Virasoro.}
    The orbifold ground-state energy
    $E_0(m) = -N/2 + m(N{-}m)/2$
    at $c_{\mathrm{orb}} = 12N^2$ (UV) flows to
    $c_3 = 12\,b(N)$ (IR) via the $c$-theorem
    (Theorem~\ref{thm:rg-monotonicity}).
  \end{itemize}
  The WDW kinetic coefficient $c_4$ is then
  \emph{forced}: if $c_4 \ne c_1 = c_2 = c_3$,
  the FK ground state would disagree with the
  boundary CFT vacuum, contradicting the CS/WZW
  correspondence (\citealt{Witten1988}).
  The system is overdetermined---three independent
  equations for one unknown, each yielding
  $c = 12\,b(N)$~\eqref{eq:central-charge}.
