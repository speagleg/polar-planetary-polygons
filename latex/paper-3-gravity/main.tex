...
the partition function $Z(k)$ is entire in~$k$ with nearest
singularity at $k = -2$ \citep{Witten1991}, well away from
$k = 2b(N) > 0$;
and the semiclassical expansion has $1/k < 2\%$ corrections
for $N \ge 7$ ($k > 8.6$).
...
Hoeffding bound $\mathrm{Var}(e^{aX}) \le a^2 e^{2a^2 \mathrm{Var}(X)} \mathrm{Var}(X)$
(valid for bounded~$X$ on a compact surface, $|X| \le \sqrt{2}$):
\[
  \mathrm{Var}(R') \;\le\;
