...
\item \emph{CS on punctured surface $\to$ KZ $\to$ CMS}
  (Etingof--Frenkel--Kirillov, Theorem~7.1.2).
...
$\mathrm{SU}(2) \times \mathrm{SL}(2,\mathbb{R})$
(Euclidean, $\Lambda < 0$).
...
\item $Z_{\mathrm{CS}}(N)$: the $\mathrm{SL}(2,\mathbb{R})$
  ...
  $\mathrm{SU}(2)$ (\citealt{Witten1991Analytic}).
...
\item[\emph{5b.}] \emph{CS is one-loop exact.}
  For compact gauge groups, the CS partition function
  around each flat connection equals the classical
  action times the Ray--Singer torsion times the
  level shift $k \to k{+}h^\vee$
  (\citealt{Witten1989Jones}, \S2.3;
  \citealt{AxelrodSinger1994}).
  For the non-compact group
  $\mathrm{SL}(2,\mathbb{R})$,
  the same structure holds for two reasons:
  ...
  require nontrivial $\pi_3$ (Witten 1989, \S3);
  they vanish here.

  \emph{(ii)~Perturbative expansion is local.}
  The saddle-point expansion of the CS path integral
  around the unique flat connection (Step~3) is a
  formal power series in $1/k$.
  Each coefficient is a Feynman integral on the
  ...
  the vanishing of higher loops for $\mathrm{SU}(2)$
  (Axelrod--Singer~1994) implies the same vanishing for
  $\mathrm{SL}(2,\mathbb{R})$.
  The continuous Plancherel spectrum of
  $\mathrm{SL}(2,\mathbb{R})$
  (principal series, $s \in \mathbb{R}$) contributes to
  the global Plancherel decomposition of
  $L^2(\mathrm{SL}(2,\mathbb{R}))$
  but does not enter the perturbative expansion
  ...
  locus is unique by Troyanov uniformization (Step~3).
  Therefore $Z_{\mathrm{CS}}$ is exactly its one-loop
...
(\citealt{GrenanderSzego1958}, Theorem~5.2)
...
surface (\citealt{GilbargTrudinger2001}, Thm.~9.11),
...
(\citealt{FeffermanGraham1985}, Theorem~1.2;
