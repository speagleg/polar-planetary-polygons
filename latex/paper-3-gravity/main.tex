...
\]
The determinant is
$\cos^2(\pi q_1) - \cos^2(\pi q_2)
= \sin^2(\pi q_2) - \sin^2(\pi q_1) \ne 0$,
...
\item \emph{Onsager contraction (constructive, finite~$N$).}
  ...
  at $N = 7$ (contraction factor $0.056$ via the sharp Sobolev bound,
  or $0.071$ via the coarser $L^\infty$ bound; explicit in both cases).
...
\item[\textup{(H2)}]
  ...
  at the threshold $N = 7$; all remaining $\lambda_m$ are
  bounded away from zero: $\min_{m \ne m^*} \lambda_m \ge c > 0$
  throughout the concavity basin),
...
giving contraction factor $(1/6)^2 \times e^{0.30} \times 1.48 = 0.056$
on the Bolza surface (Sobolev bound in the proof below;
the cruder basin estimate $\mathrm{Var} \le 2$ gives $0.071$).
...
and $a = \beta_{\mathrm{eff}}$, and using
$\mathrm{Var}(Y) \le V_1^2\,\mathrm{Var}(R)
\bigl(1 + 2|V_2/V_1|\sqrt{\mathrm{Var}(R)}\bigr)^2$
(Cauchy--Schwarz on the $V_2\,\delta R^2$ cross-term):
\[
  \mathrm{Var}(R') \;\le\;
...
$(1 + 0.250\sqrt{\mathrm{Var}})^2 \le 1.83$
for $\mathrm{Var}(R) \le 2$, giving total contraction
$(1/6)^2 \times 1.40 \times 1.83 = 0.071 < 1$.
...
$\|\delta R\|_\infty^2 \le 0.80$,
$\|Y\|_\infty \le 0.90$,
and the nonlinear factor becomes
$(1 + 0.250\sqrt{0.80})^2 = 1.50$,
tightening the contraction to
$(1/6)^2 \times e^{0.30} \times 1.50
= (1/6)^2 \times 1.35 \times 1.50 = 0.056 < 1$.
...
\textbf{Step 3: Exact result via the Robin function
(removing the $O(\varepsilon^2)$ residual).}
The leading-order argument in Step~2 gives $R = $ const up
to $O(\varepsilon^2)$; we now show the residual vanishes
exactly for any $\varepsilon > 0$ below a threshold.
By the Riemannian Havelock theorem, $C_1(x)$ admits the
heat-kernel representation
$C_1(x) = \gamma_\varepsilon(x) + b(N)$,
where $\gamma_\varepsilon(x)$ is the regularized Robin
function (the regular part of the Laplacian Green's
function at scale~$\varepsilon$; see Paper~I,
Theorem~\ref*{I-thm:riemannian-havelock} and Remark therein).
The Robin function satisfies the Gauss--Bonnet identity
(Lemma~\ref{lem:robin-identity} below)
\begin{equation}\label{eq:robin-identity}
  -\Delta\gamma(x) = K(x) - \bar K,
  \qquad \bar K = \frac{2\pi\chi}{A},
\end{equation}
and the regularized version satisfies
$-\Delta\gamma_\varepsilon(x) = K(x) - \bar K
+ R_\varepsilon(x)$ with
$\|R_\varepsilon\|_{L^2} \le C_{\mathrm{heat}}\,
\varepsilon^2\,\|\Delta K\|_{L^2}$
(Seeley 1967 heat-kernel expansion;
$C_{\mathrm{heat}}$ depends on the surface only through
area and genus).
From Step~1, $C_1 = $ const implies $\gamma_\varepsilon = $ const,
so $\Delta\gamma_\varepsilon = 0$, so
$K - \bar K = -R_\varepsilon$.
Setting $u(x) := K(x) - \bar K$, we have
$\|u\|_{L^2} \le C_{\mathrm{heat}}\,\varepsilon^2\,
\|\Delta u\|_{L^2}$.
By the standard Schauder elliptic estimate on a compact
surface (\citealt{GilbargTrudinger2001}, Ch.~6),
$\|u\|_{H^2} \le C_S (\|u\|_{L^2} + \|\Delta u\|_{L^2})$,
and $\|\Delta u\|_{L^2} \le \|u\|_{H^2}$, so
\[
  \|u\|_{L^2}
  \le C_{\mathrm{heat}} C_S \varepsilon^2
  (\|u\|_{L^2} + \|\Delta u\|_{L^2})
  \le C_{\mathrm{heat}} C_S \varepsilon^2
  (\|u\|_{L^2} + \|u\|_{L^2}/(C_{\mathrm{heat}}\varepsilon^2))
  = (C_{\mathrm{heat}} C_S \varepsilon^2 + C_S)\|u\|_{L^2}.
\]
For $\varepsilon^2 < 1/(C_{\mathrm{heat}} C_S)$ and
$C_S < 1$ (which holds uniformly on compact hyperbolic
surfaces with Sobolev constant bounded away from one),
the coefficient on the right is $< 1$, forcing
$\|u\|_{L^2} = 0$, hence $K(x) = \bar K$ exactly.
The threshold $\varepsilon^2 < 1/(C_{\mathrm{heat}} C_S)$
is satisfied since $\varepsilon$ (the Cauchy blob width,
Paper~II, Proposition~\ref*{II-prop:blob}) is a free
regularization parameter that can be taken arbitrarily
small; in the $\varepsilon \to 0$ limit, Step~3 removes
the $O(\varepsilon^2)$ residual of Step~2 and establishes
$R(x) = R_0$ exactly for all $x$.
...
By Hadamard's expansion near $y = x$,
$G(x,y) = -(1/2\pi)\log d(x,y) + \gamma(x,y)$
with $\gamma$ smooth; setting $y = x$ gives $\gamma(x)$.
Applying $\Delta_x$ to both sides: the singular part
contributes $\Delta_x[(1/2\pi)\log d(x,y)]$, which by the
conformal-factor expansion of the distance function gives
$K(x)/(2\pi)\cdot(-2\pi) = -K(x)$ at $y = x$.
The regular part contributes $\Delta_x\gamma(x)$.
The right side gives $-1/A\,\Delta_x(1) = 0$.
Integrating over $S$: $\int\Delta_x\gamma\,dA = 0$,
so $\int(K(x) - \bar K)\,dA = 0$ by Gauss--Bonnet,
consistent with exactness of $\Delta\gamma$.
The identity holds pointwise:
$-\Delta\gamma = K - \bar K$.
(Classical result; see \citealt{Okikiolu2008}, \S2 for a
modern exposition.)
\end{proof}
