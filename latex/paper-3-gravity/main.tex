...
\externaldocument[V-]{../paper-4-field-theory/main}
...
\item \emph{The WDW kinetic coefficient.}
  ...
  (\S\ref{sec:lichnerowicz}, Step~2b) produces a kinetic
...
Paper~IV Definition~5.1).
At leading order $c \approx N^2$, so $k \approx N^2/6$.
The CS level $k = c/6$ is not integer for most~$N$.
Level quantisation in CS theory arises from large gauge
transformations; for the non-compact $\mathrm{SL}(2,\mathbb{R})$,
the relevant topological constraint is weaker than for compact groups.
In the semiclassical regime $k \gg 1$ (large~$N$),
integrality corrections are~$O(1/k)$ and the CS formulation
is reliable (see Paper~IV, \S\ref*{V-sec:gauge-group},
...
by Stieltjes--Wigert polynomials \citep{Tierz2006},
which are the orthogonal polynomials of the log-normal
(Stieltjes) moment problem.
The Havelock moment sequence $r_k = \lambda_0^k/k!$
is Stieltjes (positive-definite Hankel matrix, verified
for $k \le 14$), linking the vortex spectral data
directly to the CS partition function.
...
Since the gravitational interaction (Paper~II, equation~\ref*{II-eq:grav-interaction})
...
is exactly the $1$-point function of the twist field~$\sigma_m$
on the orbifold $\mathbb{C}/\mathbb{Z}_N
\cong \mathbb{C}$ (genus zero)
\citep{DixonHarveyVafaWitten1985}.
Since $\mathbb{C}/\mathbb{Z}_N$ is topologically $\mathbb{C}$,
it has a unique vacuum state, and the $1$-point function
is entirely determined by the vacuum OPE data
(no non-vacuum intermediate states can contribute on genus zero).
Therefore the identity block dominates \emph{exactly}
in the genus-zero sector.
...
Non-identity block corrections appear at $O(e^{-c})$
(exponentially suppressed at large central charge).
...
factorial.  The frozen partition function is the Gaussian integral
$Z_{\mathrm{frozen}} = \prod_{m \ne m^*}(2\pi/\lambda_m)^{1/2}$
over the non-critical modes, with Casimir gaps
$\lambda_m(\rho^*) = (N{-}2m)^2/8$
...
The vortex stability analysis on this surface therefore
computes Chern numbers of a hyperbolic topological insulator:
the spectral bound $|\delta C_1| \lesssim 0.05$ constrains
the phase diagram of the $\{8,3\}$ Haldane model.
...
\emph{Notation.}
The potential $\log(2\sinh\rho) + b(N) - f(m,N)$ is the
\emph{unconstrained} Havelock eigenvalue in the geodesic
coordinate~$\rho$.
The \emph{constrained} eigenvalue offset used in the holographic
interpretation (\S\ref{sec:holographic}) is
$C_1(\xi) = (N{-}1)(1{+}\xi^2)/(1{-}\xi)^2$, where
$\xi = \tanh^2(\rho/2)$.
At small~$\rho$: $\log(2\sinh\rho) + b(N) \approx \log\rho + b(N)
\to C_1 \to N{-}1$; the two agree in the flat limit.
...
The Kirchhoff equations $\dot{z}_k = -i\,\partial H/\partial\bar{z}_k$
are first-order, but the breathing mode~$\rho$
(parametrising the family $z_k(\rho) = \rho\,e^{2\pi i k/N}$)
acquires second-order dynamics through the gravitational sector.
The kinetic coefficient $c = 12\,b(N)$ is the Todd offset
extracted from the eigenvalue data
(\S\ref{sec:intro-gravity}, logical architecture, step~1).
The Feynman--Kac mapping of the Onsager partition function
produces this $c$ as the effective mass of the breathing mode
(Paper~IV, \S\ref*{V-sec:qm-gr-bridge}).
...
$c_{\mathrm{orb}} = 12N^2$ (the UV orbifold central charge)
...
gravity, with $\Lambda = R_S/2$
(in $2{+}1$D, $R_{\mu\nu} = \Lambda g_{\mu\nu}$ on a static slice
gives spatial scalar curvature $R_S = 2\Lambda$).
\end{theorem}
...
\medskip
\noindent\textbf{Step~2a. The Hamiltonian constraint (H2)
from the Havelock threshold.}
On the \emph{flat plane}, the Havelock stability theorem
(Paper~I) proves that the $N$-gon is marginally stable
at $C_1 = N{-}1 = f(m^*,N)$ for $N \le 7$.
On a curved surface, the Hamiltonian constraint
$C_1(x,\varepsilon) = f(m^*,N)$ defines the
\emph{stability threshold}: the value of the polygon radius
where the critical eigenvalue $\lambda_{m^*} = 0$.
This threshold exists on \emph{any} Riemannian surface
(by continuity of $C_1$ in the ring radius), providing
(H2) as a geometric condition rather than a dynamical selection.
The WDW equation (Step~2b) then promotes this classical threshold
to a quantum constraint.

\emph{Logical dependency}: Step~1 (H1 $\Rightarrow$
logarithmic Green's function) determines the potential $V(\rho)$
in the WDW equation below.
Step~2b writes the WDW equation with this potential.
Step~3 takes the classical limit $c \to \infty$ to recover (H2).
The chain (H1) $\to$ $V(\rho)$ $\to$ WDW $\to$ (H2) is
unidirectional; (H2) is \emph{derived from} (H1) and the
Feynman--Kac mapping, not assumed independently.
...
eigenvalue data alone:
(i)~the three-layer decomposition extracts $b(N)$
as the unique mode-independent offset
(Proposition~\ref{prop:equiv-rr});
(ii)~define $c := 12\,b(N)$;
(iii)~the Onsager partition function
$Z = \int\mathcal{D}[\rho]\,e^{-S_{\mathrm{E}}}$ with
$S_{\mathrm{E}} = \int(c/2)\dot\rho^2 + V(\rho)\,d\tau$
produces the WDW equation via the Feynman--Kac theorem.
No gravitational input enters: $c$ comes from step~(i),
and the Brown--Henneaux relation $c = 3\ell/(2G)$ is a
consistency check (\S\ref{sec:intro-gravity}, step~3).
...
In the Hadamard derivation, the $1/4$ decomposes into
$1/6$ (from the curvature term $-R\,d^2/12$ in the parametrix,
with $\sum_p\sin^2(\pi p/N) = N/2$) and $1/12$ (from the
curvature correction to chord distances at $O(\varepsilon^3)$).
The exact coefficient $1/4$ matches the exact
...
In $d = 3$ (spatial), the full Weyl tensor has
$d(d{+}1)(d{+}2)(d{-}3)/12 = 10$ independent components;
...
$\ell = 2$ Weyl sector in $d = 3$, we need
$\lfloor N/2\rfloor \ge 5$, i.e.\ $N \ge 10$ (even) or
$N \ge 11$ (odd).
Verified numerically: the $5 \times 5$ cosine matrix
$M_{mp} = \cos(2\pi pm/N)$ has rank~$5$ for $N = 11$
but rank~$3$ for $N = 6, 7$ (insufficient).
Any nonzero Weyl tensor therefore makes $C_1$ depend on~$m$
for $N \ge 11$, violating~(H1).
Hence (H1) at every~$x$ with $N \ge 11$ forces $W_{ijkl} = 0$ and
$R_{ij} = (R/d)\,g_{ij}$: maximal symmetry.
Promoting to the $(d{+}1)$-dimensional spacetime:
$R_{\mu\nu} = \Lambda\,g_{\mu\nu}$.

In $d = 4$, the Weyl tensor has $10$ components at $\ell = 2$
($5$ real), requiring $\lfloor N/2\rfloor \ge 5$, i.e.\ $N \ge 11$
---the \emph{cosmological polygon} (Paper~IV, Theorem~11).
\end{proof}
