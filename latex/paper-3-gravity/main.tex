...
The identification of Regge vertices (deficit angles) with
point vortices (circulations) is not an independent hypothesis
but a corollary of three theorems:
\begin{enumerate}
\item \emph{2+1D gravity $=$ $\mathrm{SL}(2,\mathbb{R})$ CS theory}
  (\citealt{Witten1988}; conical singularities become
  punctures with fixed holonomy).
\item \emph{CS on punctured surface $\to$ KZ $\to$ CMS}
  (Etingof--Frenkel--Kirillov, Theorem~7.1.2:
  the CS constraint at the $\mathbb{Z}_N$-symmetric locus
  \emph{is} the CMS Hamiltonian).
\item \emph{CMS Casimir $=$ Havelock eigenvalue}
  (Proposition~\ref{prop:casimir-havelock}: both equal
  $m(N{-}m)/2$ from the $\mathbf{H}^2$ Laplacian).
\end{enumerate}
The composition Regge vertex $\to$ CS puncture $\to$
KZ monodromy $\to$ CMS eigenvalue $\to$ Havelock eigenvalue
is exact at every step.
The Casimir eigenvalue $C_2(m) = m(N{-}m)/2$ is a property
of the $\mathfrak{sl}(2,\mathbb{R})$ representation
(Helgason symmetric space theorem), independent of the CS
level~$k$; the fact that $k = c/6 = 2\,b(N)$ is irrational
affects the normalization of the WZW conformal weights but
not the eigenvalue identity itself.
What distinguishes vortices from other logarithmic-source
systems (electrostatics, 2D point masses) is the
\emph{first-order dynamics}: CS is first-order, Kirchhoff
vortex dynamics is first-order, and the EFK theorem
preserves this structure.
Second-order systems (Coulomb, Newtonian) do not have
a CS formulation and do not enter.
The OPS spectral determinant theorem then selects the
constant-curvature geometry as the unique entropy maximum.
All conditions (H1)--(H3) are mathematical theorems;
the $17$~testable outputs enumerated in Paper~VII
constitute the empirical test.
...
\begin{remark}[Logical status: all inputs are theorems]
\label{rmk:logical-status}
All four conditions are mathematical theorems.
...
The polygon entropy $S_{\mathrm{poly}} = \tfrac{1}{2}\sum_{m \neq m^*}
\log\lambda_m$ is a finite sum over $N{-}1$ modes.
...
\item \emph{Onsager contraction (constructive, finite~$N$).}
  ...
  proves $95\%$ variance reduction per iteration
  at $N = 7$ (contraction factor $0.049$, explicit bounds).
...
The OPS theorem (Remark~\ref{rmk:logical-status})
identifies this maximum with the physical equilibrium:
$\det'\Delta_g = \exp(S_{\mathrm{total}} + \mathrm{const})$
is uniquely maximized at constant curvature.
...
microcanonical entropy; this uses Lemma~\ref{V-lem:nonadiabatic}
from Paper~V).
...
This is a \emph{consistency check}, not a derivation:
both~$G$ values agree when the polygon radius is set
by the BO potential, with no free parameters.
\end{remark}
...
\begin{enumerate}
\item \emph{OPS, not relaxation.}
  The entropy maximization comes from the OPS spectral
  determinant theorem (the constant-curvature metric
  uniquely maximizes $\det'\Delta$), not from dynamical
  relaxation of the integrable CMS modes.
  The Onsager principle provides the physical interpretation
  (negative-temperature equilibrium) but is not the
  logical foundation.
\item \emph{Two variational principles.}
  (H2a) characterizes the entropy functional at fixed geometry;
  (H2b) selects the entropy-maximizing metric via the OPS
  theorem.  Neither implies the other.
...
The value $c = 12\,b(N)$ is uniquely determined by
three independent constraints that all give the same answer:
...
(the $\delta^2$ terms vanish because the $\mathbb{Z}_N$
