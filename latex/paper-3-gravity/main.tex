...
\item \emph{Geometric origin (Fisher--Rao).}
  ...
  The BO separation (step~1) is exact for
  \emph{any} kinetic coefficient: the Havelock
  eigenvectors are $\rho$-independent Fourier modes,
  so the angular bath decouples from the
  breathing mode regardless of~$c$.
  What $c$ determines is the semiclassical scale
  ($c \to \infty$ is the classical limit) and the
  tunnelling amplitude.
  The value $c = 12\,b(N)$ is fixed by identifying
  the FK path-integral measure coefficient with
  the boundary CFT central charge (step~3).
