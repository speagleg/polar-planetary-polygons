...
The ratio of consecutive trivial-representation eigenvalues
$\lambda_n/\lambda_{n+1} \le 15.05/23.08 = 0.65$
is verified for the first $8$ eigenvalues of
\citet{Strohmaier2013}; Weyl asymptotics
($\lambda_n \sim cn$ for large~$n$)
guarantee the ratio decreases beyond this range.
...
$\delta\rho/\rho \approx \delta V/|T| \approx |\beta|\,\delta R$.
The new curvature $R' \propto \rho$, giving
$\delta R'/R_0 \approx |\beta|\,\delta R$, hence
$\mathrm{Var}(R') = |\beta|^2 R_0^2\,\mathrm{Var}(R)$.
...
\noindent
\emph{Logical status.}
The Onsager contraction upgrades~(H2b) from a
physical hypothesis to a theorem: the self-consistent
metric is \emph{proved} to be constant-curvature,
using only~(H1), (H2a), the Onsager--Regge Lemma, and the
vortex-gravity isomorphism (deficit angle~$=$ circulation,
a kinematic identity in~$2{+}1$D).
The remaining physical input is the isomorphism itself---a
single kinematic identification, not a thermodynamic principle.
