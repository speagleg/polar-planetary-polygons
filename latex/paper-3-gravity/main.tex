...
and no competing non-identity saddle.
At finite~$c$: the non-identity corrections are bounded by
Proposition~\ref{prop:non-crossing}
($|f_m/c| \le 1/8 < 1$), which ensures no eigenvalue
sign changes.
\end{proof}
...
\paragraph{The Clausius relation from the threshold geometry.}
At each palindromic threshold (a local Rindler horizon),
the three thermodynamic quantities are \emph{determined}
by the WDW potential $V(\rho) = \log(2\sinh\rho) + b(N) - f(m^*,N)$:
\begin{itemize}
\item $T = V'(\rho^*)/(2\pi) = \coth(\rho^*)/(2\pi)$
  (Unruh temperature from the surface gravity $\kappa = V'$);
\item $\delta Q = \kappa \times A = \coth(\rho^*) \times 2\pi\sinh(\rho^*)$
  (energy flux: surface gravity $\times$ horizon circumference);
\item $dS/d\rho = V'(\rho^*)/2 = \coth(\rho^*)/2$
  (entropy production: $(1/2)\,d\log|\lambda_{m^*}|/d\rho$
  from the single zero mode).
\end{itemize}
...
\begin{equation}\label{eq:clausius}
  ...
  = 4\pi - O(e^{-2\rho^*}).
\end{equation}
The physical content: the ratio extracts an effective
gravitational coupling $G_{\mathrm{eff}} = \kappa/(8\pi)
= \coth(\rho^*)/(8\pi)$
from the threshold geometry, with $G_{\mathrm{eff}} \to 1/(8\pi)$
as $\rho^* \to \infty$.
This is \emph{derived} from the potential, not assumed.
