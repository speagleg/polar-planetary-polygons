...
\begin{itemize}
  ...
  \item The bound $|\delta C_1| \le 2/(4\pi\lambda_{\mathrm{triv}})$
    at the Aut-fixed centre
    ($\lambda_{\mathrm{triv}} \approx 15.05$ for the Bolza surface),
    or $|\delta C_1| \le 2/(4\pi\lambda_1)$ at a generic point
...
The bound $|\delta C_1| \lesssim 0.05$ shows that polygon
...
(Paper~IV, Theorem~\ref*{V-thm:anomaly-vanishes}).
...
\begin{theorem}[Polygon stability implies constant curvature]
...
\begin{proof}
The proof has three steps: (1)~force the interaction to be
logarithmic, (2)~extract $R(x)$ from the Hadamard parametrix,
(3)~conclude $R = \mathrm{const}$ from (H1) $+$ (H3).
...
\medskip
\noindent\textbf{Step~3. (H1) $+$ (H3) force $R = \mathrm{const}$.}
By (H1), $\lambda_{m^*} = C_1(x,\varepsilon) - f(m^*,N)$.
At the stability threshold ($\lambda_{m^*} = 0$):
$C_1(x,\varepsilon) = f(m^*,N)$.
This threshold exists at every~$x$ (by continuity of~$C_1$
in the ring radius and the Hadamard universality of~(H3)).
Substituting~(\ref{eq:C1-expansion-proof}):
...
(Paper~IV, Theorem~\ref*{V-thm:n-selection}).
\end{proof}
...
The partition function of the frozen angular modes is
$Z_{\mathrm{bath}}(\rho) = \prod_{m \ne m^*}
\lambda_m(\rho)^{1/2}$
(the Toeplitz determinant of the csc$^2$ kernel).
The mode-independent offset in
$\log Z_{\mathrm{bath}} = \tfrac{1}{2}(N{-}1)C_1 - \tfrac{1}{2}\Sigma f_m$
is $b(N) = N(N{+}1)/12 - \ln 2 + (\ln N)/(N{-}1)$
...
$R = \mathrm{const}$ regardless of~$d$ (Step~3).
...
\paragraph{The Clausius relation from Bisognano--Wichmann.}
At each palindromic threshold, the critical eigenvalue
$\lambda_{m^*} = 0$ partitions the angular modes into two
groups: those with $\lambda_m > 0$ (stable) and the
zero mode at the boundary.
The angular bath is a collection of free harmonic oscillators
(the Havelock modes).
By the Bisognano--Wichmann theorem for free fields,
the modular Hamiltonian of the oscillator vacuum restricted
to one side of a bipartition equals the Rindler boost
Hamiltonian.
The ``bipartition'' here is the BF crossing ($\lambda_{m^*} = 0$),
and the Rindler temperature is the Unruh temperature
$T = V'(\rho^*)/(2\pi)$.
The Jacobson thermodynamic argument then gives the full
Einstein equations with matter from $\delta Q = T\,\delta S$
at every local Rindler horizon.
