...
all three hypotheses are derived
(H1~from the $\csc^2$ uniqueness theorem,
H2~from the WDW breathing-mode constraint,
...
\item[\textup{(H1)}]
  ...
  theorem (Paper~I).
\item[\textup{(H2)}]
  \emph{Uniform threshold}: there exists $\varepsilon_0 > 0$
  such that $C_1(x,\varepsilon_0) = f(m^*,N)$ for all~$x$.
  \emph{Derived} from the WDW equation
  (Proposition~\ref{prop:wdw-lambda}):
  the breathing-mode constraint $V(\rho) = 0$ selects a
  universal equilibrium radius $\rho_0 = \varepsilon_0$
  independent of~$x$.
\item[\textup{(H3)}]
...
It is~(H2)---the uniform threshold selected by the
WDW constraint---that pins $C_1$ to the $x$-independent
value $f(m^*,N)$ and thereby forces $R = \mathrm{const}$.
Without~(H2), (H1) $+$ (H3) carry no curvature content.
With~(H2), the Hadamard expansion (Step~2) connects
