...
\textbf{(H1)} and~\textbf{(H3)} are mathematical theorems
($\csc^2$ uniqueness and Hadamard parametrix).
\textbf{(H2a)} is a theorem (strict concavity $+$ Jensen).
\textbf{(H2b)} is a theorem of spectral geometry, not a
physical hypothesis:

The polygon entropy $S_{\mathrm{poly}}$, the Liouville action
$S_L$, and the spectral determinant $\log\det'\Delta$ are
the same functional in three languages:
\[
  S_{\mathrm{poly}}[g]
  = -S_L[g] + \mathrm{const}
  = \log\det{}'\Delta_g + \mathrm{const}
\]
(the first equality is the definition of the Boltzmann entropy
for the Coulomb gas; the second is the Polyakov formula
\citep{Polyakov1981}).
The \citet{OsgoodPhillipsSarnak1988} theorem states:
among all conformal metrics on a compact surface with fixed
area, $\det'\Delta$ is uniquely maximized by the
constant-curvature metric.
This is a proved theorem (the proof uses Jensen's inequality
applied to the heat kernel, the same inequality as our~(H2a)).
Combined with the Polyakov identity, it gives: the entropy
$S_{\mathrm{poly}}$ is uniquely maximized at $R = \mathrm{const}$.

The Onsager principle \citep{Onsager1949,CagliotiFLMP1992,Kiessling1993}
provides the \emph{physical interpretation} of the OPS theorem:
the microcanonical ensemble at negative temperature
selects the entropy maximum.
But the mathematical content---that the unique entropy maximum
has $R = \mathrm{const}$---is a theorem independent of the
vortex-gravity identification.
No circularity arises: the derivation of $R = \mathrm{const}$
uses (H1)~$\csc^2$ universality, (H2a)~strict concavity,
(H2b)~OPS/Jensen, and (H3)~Hadamard---all of which are
mathematical theorems about 2D Riemannian geometry.
The OPS theorem applies to continuous metrics; the
$N$-point polygon functional differs from the continuous
limit by $O(\varepsilon^2) = O(1/c)$, where
$c = 12\,b(N)$ controls the point-vortex regularization
(Laplace method, $O(1/c) \approx 2\%$ at $N = 7$;
\citealt{Erdelyi1956}, \S2.4).
The Onsager contraction (Corollary~\ref{cor:onsager-contraction})
provides the independent finite-$N$ bound: $97\%$ variance
reduction per iteration at $N = 7$.
\end{remark}
...
\item[\textup{(H2)}]
  ...
  \textbf{[strict concavity $+$ OPS spectral determinant]}:
  ...
  (Jensen's inequality).
  The identification of the entropy maximum with
  the physical equilibrium follows from the
  Osgood--Phillips--Sarnak theorem
  \citep{OsgoodPhillipsSarnak1988}: the spectral
  determinant $\det'\Delta_g$ --- which equals
  $\exp(S_{\mathrm{poly}})$ up to a constant
  (Polyakov formula) --- is uniquely maximized at
  constant curvature.
  This is a theorem of spectral geometry, not a
  physical hypothesis (Remark~\ref{rmk:logical-status}).
\item[\textup{(H3)}]
