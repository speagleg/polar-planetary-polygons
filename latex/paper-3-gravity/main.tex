...
(ii)~the mean aliasing
$\langle D(N)\rangle = N(N{+}1)/12 + \log N/(N{-}1)$
(exact closed form, Paper~I Proposition~\ref*{I-prop:growth-closed-form});
(iii)~the growth law $a_N \equiv \rho^*(N)/f_{\mathrm{crit}}(N) =
  1/3 - 2/(3N) + 8\log 2/N^2 - 8\log N/N^3 + O(N^{-3})$
(exact asymptotic expansion, same reference);
...
Predictions (ii) and (iii) were previously stated only to leading
order; they are now known in closed form via three elementary
identities (cosine orthogonality, the log-sine product
$\prod_{p=1}^{N-1} 2\sin(\pi p/N) = N$, and
