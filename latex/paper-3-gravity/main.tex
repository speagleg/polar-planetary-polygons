...
the basin contains all metrics with
$\max|R - R_0| \le \sqrt{2}$---a large neighbourhood.
Any compact surface of genus $g \ge 2$ admits a unique
uniformised metric with $R = \mathrm{const}$ (Poincar\'e
uniformisation); the Onsager iteration starts from a
perturbation of this uniformised metric, and
$\mathrm{Var}(R) \le 2$ is satisfied by any metric in the
$H^2$ Sobolev ball of radius~$\sqrt{2}$ around the uniformised
metric---a metrically thick neighbourhood that contains all
