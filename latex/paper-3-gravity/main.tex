...
algebras is a \emph{spectral isomorphism}
(eigenvalue matching on each $\mathbb{Z}_N$ Fourier mode, not a full representation isomorphism).
...
\emph{No mode mixing occurs}: at the $\mathbb{Z}_N$-symmetric
point, the KZ connection commutes with the $\mathbb{Z}_N$
action (the connection is $\mathbb{Z}_N$-equivariant
at a $\mathbb{Z}_N$-fixed configuration), so it
preserves each $\mathbb{Z}_N$ irreducible representation.
Since each Fourier mode $V_m$ is
one-dimensional (for $m \ne N{-}m$; the palindromic pair
shares a two-dimensional eigenspace with the
...
spectral isomorphism (eigenvalue matching on each mode), not a spectral accident.
\end{remark}
...
The discrepancy reflects the integrability of the CMS
bath (the higher integrals constrain the response beyond
...
\[
  ...
  + \frac{\ln N}{N{-}1}
\]
(proved analytically in Paper~I, \S\ref*{I-sec:algebraic-structure},
via the Gauss product $\prod_{p=1}^{N-1}\sin(\pi p/N) = N/2^{N-1}$).
...
The CL Gaussian approximation motivates the kinetic term
but does not determine $c$.
No dynamical mechanism independently produces the value
$c = 12\,b(N)$: the CL mechanism confirms $c > 0$ but gives
a numerically different coefficient.
The exact value $c = 12\,b(N)$ is determined by the spectral
data of the Havelock decomposition (the Gauss product formula),
not by a dynamical derivation.
\emph{The value $c = 12\,b(N)$ is not circular}: $b(N)$
is proved analytically from the Gauss log-sine product
(Paper~I, \S\ref*{I-sec:algebraic-structure}),
giving $b(N) = N(N{+}1)/12 - \ln 2 + (\ln N)/(N{-}1)$
by pure mathematics.
The identification $c_{\mathrm{Havelock}} = c_{\mathrm{BH}}$
(the Brown--Henneaux central charge) is a physical assumption---the
same vortex--gravity isomorphism that underlies the entire framework.
It is tested by the four-role consistency check above, not assumed
independently.
The central charge $c = 12\,b(N)$ is \emph{defined}
as $12\times$ the mode-averaged eigenvalue offset.
The orbifold matching (role~4) is then a
\emph{consistency check}, not the source of~$c$.
...
Paper~I, \S\ref*{I-sec:kk-programme}.)  This is mode-independent (the \emph{same}~$c$ for
...
\item \textbf{(H2) has two layers:}
  ...
  \item[\textbf{(H2b)}] \textbf{Onsager selection
    ...
    This extension from a fixed surface to the metric itself
    is proved by the Onsager contraction
    (Corollary~\ref{cor:onsager-contraction}):
    the iteration converges geometrically to $R = \mathrm{const}$,
    using only the vortex-gravity isomorphism
    (a kinematic identity in~$2{+}1$D).
    The Onsager principle is not a thermalization
    claim and does not require the thermodynamic limit
    $N \to \infty$.
    At finite~$N$, the polygon partition function has a
    saddle-point controlled by~$c = 12\,b(N) \approx N^2$
    (the central charge, not the vortex count):
    $Z \propto e^{c\,S_{\mathrm{poly}}(\rho^*)}
    [1 + O(1/c)]$.
    At $N = 7$: $c = 52$, giving a $2\%$ saddle-point error.
    The saddle-point approximation at finite $N$ is the standard Laplace method for integrals with a large parameter in the exponent (\citealt{Erdelyi1956}, \S2.4), not the mean-field limit $N \to \infty$. The partition function $Z = \int e^{c\,S_{\mathrm{poly}}(\rho)}\,d\rho$ has the large parameter $c = 12\,b(N) = 52$ at $N = 7$; the Laplace error is $O(1/c) \approx 2\%$ regardless of $N$. The cited references (Caglioti et al.\ 1992, Kiessling 1993) prove the mean-field limit; the finite-$N$ saddle-point is a weaker, standard result.
    (The BO exactness---zero non-adiabatic coupling---eliminates the
    leading BO correction but does not further reduce the $O(1/c)$
    saddle-point error.)
...
compact Regge metrics by:
(i)~compute the entropy-maximising polygon on~$(S,g)$
(exists by the Onsager--Regge Lemma);
(ii)~read off its deficit angles (vortex-gravity isomorphism);
(iii)~define~$g' = T(g)$ as the resulting Regge metric.
Then the curvature variance contracts:
\begin{equation}\label{eq:onsager-contraction}
  \mathrm{Var}(R(T(g)))
  \le |\beta|^2 R_0^2\,\mathrm{Var}(R(g)),
\end{equation}
where $\beta = -(N{-}1)/E$ is the inverse negative temperature
and $R_0$ is the mean curvature.
At~$N = 7$: $|\beta| R_0 \approx 0.15$,
giving $98\%$ variance reduction per step
(convergence in~${\sim}2$ iterations).
\end{corollary}

\begin{proof}
The Onsager equilibrium at temperature~$T < 0$ distributes
vortices with density $\rho(x) \propto \exp(V(x)/|T|)$,
where $V$ is the polygon energy.
Since $V$ depends on $R$ through~$C_1$:
for a perturbation~$\delta R$ around mean~$R_0$,
$\rho(x) = \rho_0\exp\!\bigl((V_1\,\delta R
+ \tfrac{1}{2}V_2\,\delta R^2)/|T|\bigr)$
where $V_1 = \coth(\rho_*)$ and
$V_2 = -\operatorname{csch}^2(\rho_*)$.
The new curvature $R' \propto \rho$ gives
(For $N$ equal circulations~$\kappa$, each deficit angle
$\delta_k = 8\pi G\kappa$ is identical.
The mollified Regge curvature
$R_\varepsilon = \delta \times \sum_k K_\varepsilon(x - x_k)
= \delta \times \rho_\varepsilon$
is therefore \emph{exactly} proportional to the vortex density.
The contraction operates on the spatial uniformity of the
$N$~positions, not on varying deficit angles.)
\[
  \mathrm{Var}(R') \;\le\;
  |\beta|^2 R_0^2\,\mathrm{Var}(R)
  \times \bigl(1 + 2|V_2/V_1|\sqrt{\mathrm{Var}(R)}\bigr)^2.
\]
At~$N = 7$: $|\beta| R_0 = 0.15$,
$|V_2/V_1| = 0.12$.
The nonlinear factor
$(1 + 0.25\sqrt{\mathrm{Var}})^2 \le 1.83$
for $\mathrm{Var}(R) \le 2$, giving total contraction
$0.022 \times 1.83 = 0.040 < 1$.
The condition $|\beta| R_0 < 1$ holds whenever
$|T| > R_0$ (always satisfied in the Onsager regime
at $|T| \sim N^2 \gg R_0$).
...
\noindent
\emph{Logical status (non-circularity of the contraction).}
The iteration~$T$ uses the Onsager principle for vortices
on a \emph{fixed} metric---a proved theorem
\citep{CagliotiFLMP1992,Kiessling1993}.
The conclusion is that the \emph{metric itself} converges
to constant curvature.
These are \emph{different} statements:
the input is about vortex configurations;
the output is about geometry.
The contraction bridges the gap by the same logic as
Picard iteration: $T$~is defined using a known map (Onsager
on a given surface), and the contraction bound proves
the fixed-point equation $g^* = T(g^*)$ has a solution
with $R = \mathrm{const}$.
Using the Onsager principle in the definition of~$T$
is the \emph{proof method}, not a circular assumption.
The remaining physical input is the vortex-gravity
isomorphism (deficit angle~$=$ circulation,
a kinematic identity in~$2{+}1$D).
...
\medskip
\noindent\emph{Step~1 (Hadamard extraction).}
...
\medskip
\noindent\emph{Step~3 (Schur promotion to Einstein).}
