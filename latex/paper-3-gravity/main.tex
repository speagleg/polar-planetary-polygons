...
On this $2$D space, the saddle is unique: the Hessian of the
classical action (the constrained Havelock eigenvalue $\lambda_m$)
is non-degenerate at the symmetric conical metric, because
$\lambda_m > 0$ for all $m \ne m^*$ at the threshold
(the non-crossing proposition: $|f_m/c| \le 1/8$,
so no parasitic zero exists).
A second saddle would require $\lambda_m = 0$ for some
$m \ne m^*$, which the non-crossing bound excludes.
...
\medskip
\noindent\textbf{Step~2b. Why second-order dynamics from a first-order system.}
The Kirchhoff equations $\dot{z}_k = -i\,\partial H/\partial\bar{z}_k$
are first-order: the coordinates $(z_k, \bar{z}_k)$ are
\emph{canonically conjugate to each other} (there are no
separate momenta).
The breathing mode $\rho$ (parametrising
$z_k = \rho\,e^{2\pi ik/N}$) is a \emph{collective coordinate}
of this first-order system, not a canonical variable.
The $N{-}2$ angular modes form a \emph{thermal bath} for~$\rho$:
their fluctuations drive $\rho$ as Brownian motion in the
potential~$V(\rho)$.
The Caldeira--Leggett conditions are verified:
(i)~the angular modes are \emph{harmonic}
($H = V(\rho) + \tfrac{1}{2}\sum\lambda_m(\rho)|a_m|^2$,
exact Fourier diagonalisation of the circulant);
(ii)~the coupling is \emph{linear} and \emph{uniform}:
$\partial\lambda_m/\partial\rho = C_1'(\rho) = \coth(\rho)$
for all~$m$ (mode-independence of~$C_1$);
(iii)~\emph{timescale separation}: tunnelling ($\tau \sim e^{S_{\mathrm{BO}}}$)
vs.\ angular oscillation ($\tau_m \sim 1$).
The effective stochastic dynamics is overdamped Langevin:
$\gamma\,\dot\rho = -V'(\rho) + \eta(\tau)$,
with noise from the angular bath and friction from FDT
(automatic for harmonic bath with linear coupling).
The Onsager--Machlup path probability is
$P[\rho(\tau)] \propto \exp(-S_{\mathrm{OM}})$ with
\[
  S_{\mathrm{OM}} = \int_0^\beta
  \Bigl[\frac{c}{2}\,\dot\rho^2 + V(\rho)\Bigr]\,d\tau,
  \qquad c = \frac{\gamma}{2T}.
\]
The $\dot\rho^2$ term is the \emph{noise kernel}---it
measures how improbable a given velocity is in the
thermal bath---not a kinetic energy of the deterministic system.
This is the standard mechanism by which a first-order classical
system acquires second-order quantum dynamics
(the Caldeira--Leggett model for quantum Brownian motion).

The value $c = 12\,b(N)$ is determined by
\emph{self-consistency}: the WDW equation must reproduce the
Havelock spectrum.
The equivariant index
(Proposition~\ref{prop:equiv-rr}) gives $b(N)$ as the unique
mode-independent offset, and $c = 12\,b(N)$ is the unique
coefficient reproducing the correct eigenvalue structure.
This is topological ($\rho$-independent) and matches
$c \approx N^2$ at leading order
(the $N{-}1$ frozen modes each enhance the effective
mass by ${\sim}\,N$, consistent with $c = N^2 + O(N)$).

The transfer matrix $T = e^{-\epsilon\,H_{\mathrm{WDW}}}$
of this Onsager--Machlup path integral satisfies
$H_{\mathrm{WDW}}\Psi = 0$ in the dominant eigenstate
(the Feynman--Kac theorem).
No gravitational input enters: $c$ is determined by the
eigenvalue data, and the Brown--Henneaux relation
is a consistency check (\S\ref{sec:intro-gravity}, step~3).
...
Hence (H1) at every~$x$ with $N \ge 10$ forces $W_{ijkl} = 0$ and
...
($5$ real), requiring $\lfloor N/2\rfloor \ge 5$, i.e.\ $N \ge 10$
(even) or $N \ge 11$ (odd)---the \emph{cosmological polygon}
(Paper~IV, Theorem~11).
\end{proof}
...
\item $dS/d\rho = V'(\rho^*)/2 = \coth(\rho^*)/2$
  (entropy production from the single zero mode:
  $S = -\tfrac{1}{2}\log|\lambda_{m^*}|$ gives
  $dS/d\rho = -(1/2)\,\lambda'_{m^*}/\lambda_{m^*}$;
  at the threshold $\lambda_{m^*} \to 0$ with
  $\lambda'_{m^*} = C_1'(\rho^*) = V'(\rho^*) = \coth(\rho^*)$,
  the regularised limit is $dS/d\rho = \coth(\rho^*)/2$).
\end{itemize}
