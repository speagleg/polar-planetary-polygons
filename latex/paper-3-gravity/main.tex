...
On this $2$D space, the symmetric conical metric
is a non-degenerate saddle: its Hessian (the constrained
Havelock eigenvalue) satisfies $\lambda_m > 0$ for all
$m \ne m^*$ at the threshold.
A competing saddle on $\mathcal{M}_{0,N+1}^{\mathbb{Z}_N}$
would require a different conical metric where
$\lambda_m = 0$ for some $m \ne m^*$.
By Proposition~\ref{prop:non-crossing},
the $O(1/c)$ Virasoro corrections satisfy
$|\delta f_m/\lambda_m| < 1/4$ for all non-critical~$m$,
so no eigenvalue can change sign at finite~$c$.
For $N \le 4$, the reduced moduli space can be
analysed directly (genus-zero conformal blocks);
for $N > 4$, the non-crossing bound provides
the eigenvalue-level exclusion but a global
convexity argument on $\mathcal{M}^{\mathbb{Z}_N}$
