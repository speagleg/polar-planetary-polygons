...
\medskip
\noindent\textbf{Step~2. Canonical quantisation gives the WDW equation.}
The polygon dynamics on the angular-impulse surface
$\{J = \mathrm{const}\}$ reduces to a single degree of freedom~$\rho$
(the geodesic radius) with potential
$V(\rho) = C_1(x,\rho) - f(m^*,N)$.
Canonical quantisation with effective Planck mass $c = 12\,b(N)$
gives the Wheeler--DeWitt equation~(\ref{eq:wdw}).
(The Laplacian operator ordering $-d^2/d\rho^2$ is the natural
choice for the geodesic coordinate~$\rho$; alternative orderings
differ at $O(1/c)$ but the classical limit $c \to \infty$ in Step~3
is ordering-independent.)

\medskip
\noindent\textbf{Step~3. Classical limit yields the Hamiltonian constraint.}
In the limit $c \to \infty$: the kinetic term
$-\hbar^2\Psi''/(2c)$ vanishes and the WDW equation reduces to
$V(\rho) = 0$, i.e.\ $C_1(x,\varepsilon) = f(m^*,N)$.
This is condition~(H2)---the Hamiltonian constraint
$H_{\mathrm{ADM}} = 0$ of the minisuperspace.
