...
The WDW potential uses the \emph{unconstrained} form
$C_1(\rho) = \log(2\sinh\rho) + b(N)$ (geodesic coordinate).
The holographic sections use the \emph{constrained} form
$C_1(\xi) = (N{-}1)(1{+}\xi^2)/(1{-}\xi)^2$ (eq.~\ref{eq:C1-H2})
with $\xi = \tanh^2(\rho/2)$.
These are the same function in different coordinates:
$C_1(\rho) = C_1(\xi(\rho))$, coinciding at
$\rho \to 0$: $C_1 \to N{-}1$.
