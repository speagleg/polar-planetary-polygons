...
\item \emph{The coefficient $c = 12\,b(N)$ is derived from
  ...
  \emph{Why the CL coefficient differs from $12\,b(N)$.}
  The CL noise kernel produces a kinetic coefficient
  $c_{\mathrm{CL}} \sim N^2\rho^{*2}/((N{-}2)\coth^2\rho^*)$,
  which is approximately $1\%$ of~$12\,b(N)$ at $N = 7$
  (a factor of $\sim\!100$, not a small correction).
  This large discrepancy reflects the fact that $c_{\mathrm{CL}}$
  and $c = 12\,b(N)$ arise from \emph{different physical
  mechanisms}:
  \begin{itemize}
  \item $c_{\mathrm{CL}}$ is the dynamical inertia of~$\rho$
    from integrating out the angular bath in the Gaussian
    (free-field) approximation.
    It proves the kinetic term \emph{exists} ($c > 0$).
  \item $c = 12\,b(N)$ is the \emph{central charge} of the
    boundary CFT, determined by the Brown--Henneaux formula
    $c = 3\ell/(2G)$ and confirmed by the equivariant
    Riemann--Roch matching to the orbifold ground-state energy.
    It determines the \emph{value} of the kinetic coefficient
    in the holographic (WDW) formulation.
  \end{itemize}
  The holographic value $c = 12\,b(N)$ incorporates the
  \emph{full} boundary-mode structure (all CMS integrals of
  motion, not just the Gaussian spectral density), which is
  why it exceeds $c_{\mathrm{CL}}$ by two orders of magnitude.
  No parameter is fitted: $b(N)$ is proved analytically
  (Paper~I, Theorem~5.1) and the four roles of~$c$ (WDW
  coefficient, CS level, Brown--Henneaux, orbifold matching)
  are independently consistent.
\end{enumerate}
