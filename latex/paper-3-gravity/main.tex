...
three hypotheses
(H1~from the $\csc^2$ uniqueness theorem,
H2~from entropy maximisation of the frozen polygon modes,
...
\item[\textup{(H1)}]
  ...
  \emph{Proved in $d = 2$} by the $\csc^2$ uniqueness
  theorem (Paper~I).
\item[\textup{(H2)}]
  \emph{Entropy equilibrium}: the frozen-mode entropy
  $S(x) = \tfrac{1}{2}\sum_{m \ne m^*}\log\lambda_m(x,\varepsilon)$
  is maximised over the surface --- i.e., the polygon system
  is in thermodynamic equilibrium.
  \emph{Derived} from the Onsager selection principle
  (Paper~I, \S5): among all configurations consistent with
  fixed total energy, the maximum-entropy state has
  $C_1(x,\varepsilon_0) = f(m^*,N)$ uniformly in~$x$
  (the constrained optimum distributes curvature equally
  at every point).
\item[\textup{(H3)}]
  \emph{Hadamard universality}: condition~\textup{(H1)} holds
  at every point~$x \in S$.
  \emph{Automatic}: the Hadamard parametrix
