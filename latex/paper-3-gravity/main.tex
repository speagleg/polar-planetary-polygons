...
$R_{\mu\nu} = \Lambda g_{\mu\nu}$:
entropy maximisation of the frozen polygon modes on a
general background forces constant curvature, through
three conditions, \emph{all of which are theorems}:
H1~(the $\csc^2$ uniqueness),
H2~(equidistribution of the integrable angular bath on
Liouville--Arnold tori), and
H3~(Hadamard universality).
The physical content of~H2 is:
``$N{-}2$ coupled oscillators equidistribute on their
Liouville torus''---a consequence of the CMS integrability
of the Havelock system.
In $d \ge 3$, the characterisation extends (the Havelock
condition is equivalent to vacuum Einstein,
providing a new criterion for Einstein manifolds).
The Bernoulli number $B_2 = 1/6$ organises the spectral
structure (Paper~I): the stability polynomial
$6B_2(x) = P_2(2x{-}1)$ places the threshold at the
Gauss--Legendre quadrature nodes, and the $\csc^2$ kernel
connects the vortex spectrum to integrable systems from
random matrix theory to gauge theory.
\end{abstract}
...
The Thomson vortex system IS the
...
algebras is a \emph{representation isomorphism}, not merely an
algebra-level match.
...
space.
By the EFK theorem, these operators \emph{coincide}
at the $\mathbb{Z}_N$-symmetric point, so their eigenvalues
agree: both equal $m(N{-}m)/2$ on the $m$-th Fourier mode.
This is a stronger statement than ``the Casimirs happen to
match'': it says the \emph{operators themselves}
(not just their eigenvalues) are the same differential
operator on $\mathbf{H}^2$, evaluated at the polygon ring.
(Note: each $\mathbb{Z}_N$ Fourier mode $V_m$ is
one-dimensional, and the eigenvalue $m(N{-}m)/2$ has
\emph{multiplicity one} for $m = 1, \ldots, \lfloor N/2\rfloor$
(distinct values, with the palindromic pair $m$ and $N{-}m$
sharing the same eigenspace).
No representation-theoretic structure beyond the eigenvalue
is needed on a 1D space; the identification
is at the level of the \emph{operator}, not
the representation.)
Verified for $N = 3,\ldots,12$
(\texttt{proofs/cms\_cs\_isomorphism.py}).

\emph{Why this is not ``numerology.''}
One might object that operator coincidence on a mathematical
space does not establish physical identification---that
the vortex modes and the gravitational modes are
``two different systems with the same spectrum.''
In $2{+}1$D, this objection fails:
...
ARE the polygon modes (Paper~IV, Theorem~\ref*{IV-thm:graviton-4d}).
There is not ``a vortex system'' and ``a gravity system''
that happen to share a spectrum.
There is \emph{one system}---the boundary theory of
$2{+}1$D CS gravity---described in two equivalent languages
(Kirchhoff/Havelock and Chern--Simons/WZW).
The CMS/CS operator coincidence is the mathematical expression
...
Paper~I, verified to $10^{-16}$ for $N = 3,\ldots,24$).
...
Two independent derivations combine:
\begin{enumerate}
\item \emph{The kinetic term exists}
  (Caldeira--Leggett mechanism,
  Proposition~\ref{prop:wdw-lambda}):
  the $N{-}2$ angular modes form a harmonic bath whose
  exact influence functional (Gaussian, linear coupling)
  produces a $\dot\rho^2$ noise kernel in the
  Onsager--Machlup path integral.
\item \emph{The coefficient $c = 12\,b(N)$ is derived from
  the equivariant Riemann--Roch theorem}
  (Proposition~\ref{prop:equiv-rr}).
  The equivariant index of the $\mathbb{Z}_N$ orbifold gives
  $\mathrm{ind}_m = f(m,N) + b(N)$
  (the Havelock Casimir plus the Todd-class offset).
  In the orbifold CFT, the twisted-sector ground-state energy is
  $E_0(m) = -c/(24N) + cm(N{-}m)/(24N^2)$.
  Matching $\mathrm{ind}_m = E_0(m)$ for \emph{all}~$m$
  simultaneously forces $c = 12\,b(N)$ exactly---including
  the sub-leading $N/12$, $\ln 2$, and $(\ln N)/(N{-}1)$ terms,
  which are topological (Todd class) contributions,
  not regularisation artifacts.
  This is the \emph{unique} central charge compatible with the
  Havelock eigenvalue structure.
  \emph{Why the CL coefficient differs.}
  The CL noise kernel produces a kinetic coefficient
  $\sim N^2\rho^{*2}/((N{-}2)\coth^2\rho^*)$, which does not
  equal $12\,b(N)$ at any~$N$.
  This is \emph{not} a contradiction: the CL mechanism derives
  the \emph{existence} of the $\dot\rho^2$ term (from the
  bath's influence functional), while the equivariant RR
  determines its \emph{coefficient} (from the eigenvalue
  structure).  The two derivations answer different questions:
  CL asks ``does the breathing mode have a kinetic term?''
  (yes), and RR asks ``what is the coefficient?'' ($c = 12\,b(N)$).
  The CL coefficient is the \emph{bare} value before
  renormalisation by the bath modes; the RR value is the
  \emph{exact} coefficient including all mode contributions.
  No parameter is fitted: $b(N)$ is proved analytically
  (Paper~I, Theorem~5.1) and $c = 12\,b(N)$ follows from
  the index theorem.
\end{enumerate}
...
The Thomson vortex system IS the Calogero--Moser--Sutherland
model at coupling $g = 1$ (the Havelock interaction matrix
$M_{jk} = 1/\sin^2(\pi(j{-}k)/N)$ is exactly the CMS
Hamiltonian; see \citealt{Calogero1975}).
...
\textbf{All three conditions are theorems}, not assumptions:
\begin{itemize}
...
\item \textbf{(H2) is a theorem}
  (equidistribution on the integrable angular bath):
  the Havelock diagonalisation decomposes the angular modes
  into $N{-}2$ uncoupled oscillators---the CMS integrable system
  (Paper~I, Remark~3.1; \S\ref{sec:three-layer}).
  By the Liouville--Arnold theorem \citep{Arnold1978},
  the motion of this classical integrable system is confined
  to invariant tori in phase space, with quasi-periodic flow.
  For non-resonant frequencies (a full-measure set of initial
  conditions), the Weyl equidistribution theorem gives:
  \emph{time averages equal torus averages}.
  The torus average is the microcanonical measure constrained
  by all conserved quantities---the \emph{classical generalised
  Gibbs ensemble} (GGE)
  \citep{Arnold1978,Yuzbashyan2016,Spohn1991}.
  (This predates and is \emph{stronger} than the quantum GGE
  of \citet{Rigol2007}: the classical trajectory is
  \emph{exactly} confined to the torus, whereas quantum GGE
  describes relaxation of expectation values.)
  The conserved quantities of the CMS system are the
  action variables $I_m$ ($m = 1, \ldots, N{-}2$) from
  the Magri bihamiltonian hierarchy (\S\ref{sec:three-layer}).
  The classical GGE equilibrium of the angular bath,
  combined with the Caldeira--Leggett mechanism
  (\S\ref{sec:wdw}), drives the breathing mode~$\rho$
  to the state that maximises
  $S[g] = \log Z[g]$
  (the one-loop effective action of the polygon theory)
  subject to fixed total energy $\int V\,dA = E$.
  The entropy $S[g]$ is \emph{strictly concave} in~$R(x)$
  (proved below), so the unique maximum is
  $R(x) = \mathrm{const}$.
  This is \emph{not} an inference principle---it is a
  \emph{physical consequence} of equidistribution on
  Liouville--Arnold tori, which is a theorem of classical
  mechanics \citep{Arnold1978,Yuzbashyan2016}.
\end{itemize}
The theorem therefore has the structure:
\emph{three theorems $\Rightarrow$ Einstein equations.}
The physical content of~(H2) is:
\textbf{``$N{-}2$ coupled oscillators equidistribute on their
Liouville torus''}%
---a consequence of the integrability of the CMS/Havelock
system (Liouville--Arnold $+$ Weyl equidistribution),
not an inference principle or a physical hypothesis.
}}
\medskip
...
\item[\textup{(H2)}]
  \emph{Entropy maximisation} \textbf{[theorem]}:
  the $N{-}2$ angular Havelock modes form a classical
  integrable (CMS) system.  By the Liouville--Arnold
  theorem \citep{Arnold1978}, the motion is confined to
  invariant tori; the Weyl equidistribution theorem gives
  time average~$=$ torus average for non-resonant
  frequencies.  This torus average is the classical GGE
  \citep{Arnold1978,Yuzbashyan2016},
  which maximises $S[g] = \log Z[g]$ (the one-loop
  effective action) subject to fixed total energy
  $\int V(\rho(x))\,dA = E$
  and the CMS action variables.
  The unique maximum is $R(x) = \mathrm{const}$
  (from strict concavity of~$S$ in~$R$;
  \S\ref{sec:einstein-derivation}).
\item[\textup{(H3)}]
...
at every order.
The coefficient $1/4$ is verified directly from the exact
$\mathbf{H}^2$ formula: expanding
$C_1/(N{-}1) = (1{+}\xi^2)/(1{-}\xi)^2$ for small
$\xi = \tanh^2(\varepsilon/2) \approx \varepsilon^2/4$ gives
$C_1/(N{-}1) \approx 1 + \varepsilon^2/2 = 1 - R\varepsilon^2/4$
(since $R = -2$ on unit $\mathbf{H}^2$).
The Hadamard parametrix on a 2D surface gives
$G(x,y) = -(1/2\pi)\log d - (R/12\pi)d^2 + O(d^4)$
(Garabedian 1964, \S9.3).
Summing the $O(d^2)$ term over the $N$-ring with
$\sum_p\sin^2(\pi p/N) = N/2$ gives the $1/6$ contribution;
the $1/12$ arises from the curvature correction to chord
distances ($d_{\mathrm{chord}} \ne d_{\mathrm{geodesic}}$
at $O(\varepsilon^3)$).  Together: $1/6 + 1/12 = 1/4$.

\medskip
\noindent\textbf{Step~3. (H2) $+$ Step~2 force $R = \mathrm{const}$.}
By~(H2), entropy maximisation forces
$C_1(x,\varepsilon)$ to be $x$-independent
(the uniform-$C_1$ configuration maximises $S_{\mathrm{total}}$
by Jensen's inequality and the concavity of~$S$;
see below).
...
\emph{Second, independent route to\/ \textup{(H2)}:
the vortex-gravity isomorphism.}
The Liouville--Arnold derivation above is self-contained
(it uses only the integrable structure and energy conservation).
The vortex-gravity isomorphism provides a \emph{second},
independent physical picture that reinforces the result
and extends it to the dynamical setting.
In this picture, one might object that the Onsager principle
extremises vortex \emph{configurations} at fixed background
geometry, not the geometry itself.
The resolution comes from the vortex-gravity isomorphism
(Paper~II, Proposition~\ref{II-prop:grav-stability}):
in $2{+}1$D, the linearised gravitational interaction between
point masses is \emph{identical} to the point-vortex interaction,
with $\kappa \leftrightarrow 8\pi Gm$.
Deficit angle $= 2\pi\kappa$, so the metric degrees of freedom
(spatial curvature at each point) ARE vortex degrees of freedom.
...
The derivation is \emph{not} circular: it uses the
\emph{proved} vortex-gravity isomorphism (that deficit
angles~$=$ circulations) together with the Onsager
selection principle (that the maximum-entropy configuration
is selected at negative temperature).
...
The Onsager principle on this $V$-vortex system
extremises entropy over vertex positions, which
varies the Regge metric.
In the continuum limit $V \to \infty$:
$S_{\mathrm{Regge}} \to S_{\mathrm{total}}[g]$,
and $\delta S_{\mathrm{Regge}}/\delta(\text{vertices})
= 0$ converges to $\delta S[g]/\delta g = 0$.
No new physical assumption enters: the Regge
approximation is a mathematical theorem, and the
vortex-gravity isomorphism applies to each vertex
individually.
...
(Proposition~\ref{prop:partition-function}).
...
$\ell = 2$ Weyl sector in spatial dimension $d = 3$
...
for $N \ge 10$, violating~(H1).
Hence (H1) at every~$x$ with $N \ge 10$ forces the
traceless Ricci tensor $R_{ij} - (R/d)\,g_{ij} = 0$
on each spatial slice (in $d = 3$ the spatial Weyl
tensor vanishes identically; the polygon probes the
$5$ independent traceless Ricci components).
...
The $N{-}2$ angular modes form a thermal bath whose fluctuations
drive~$\rho$ stochastically (Caldeira--Leggett mechanism).
