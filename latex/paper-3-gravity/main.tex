...
In $2{+}1$D, the unique first-order topological gauge theory
with gauge group $\mathrm{SL}(2,\mathbb{R})$ is CS theory
(\citealt{Witten1988}).
The CS boundary dynamics on a punctured surface is the KZ
connection (CS/WZW theorem), which at the $\mathbb{Z}_N$
locus is the CMS Hamiltonian (EFK, Theorem~7.1.2).
The CMS Casimir $m(N{-}m)/2$ is polynomial in~$m$ (degree~$2$).
No other $2{+}1$D topological theory has this property:
higher-spin or non-compact gauge groups change the Casimir
from a degree-$2$ polynomial to a higher-degree or
...
\item[\textup{(H2)}]
  ...
  $S(x) = \tfrac{1}{2}\sum_{m \ne m^*}\log\lambda_m(x)$
  is a finite sum over $N{-}1$ modes,
...
giving contraction factor $(1/6)^2 \times 1.75 = 0.049$,
i.e.\ $95\%$ variance reduction per step.
