...
The spectral bound is rigorous: the geometric ratio
$\lambda_n/\lambda_{n+1} \le 0.65$ gives
...
\item \emph{OPE coefficient bound (Liouville action):}
  ...
  $(N{-}1)\exp(-b(N)\ln N) < 0.15\%$ for $N \ge 7$
...
factorial (even~$N$; for odd~$N$, $Z_{\mathrm{frozen}}$ is computed
directly from the Casimir gaps
...
By~(H2), $C_1(x,\varepsilon_0) = f(m^*,N)$ at every~$x$
for the universal threshold radius~$\varepsilon_0$.
Substituting the expansion~(\ref{eq:C1-expansion-proof})
at $\varepsilon = \varepsilon_0$:
\[
  (N{-}1)\!\left[1 - \frac{R(x)\,\varepsilon_0^2}{4}
  + O(\varepsilon_0^4)\right] = f(m^*,N)
  \quad\text{for all } x.
\]
The right side is $x$-independent.
Therefore $R(x) = R_0$ for all~$x$, with
$R_0 = -4[f(m^*,N) - (N{-}1)]/[(N{-}1)\varepsilon_0^2]$.
...
Hypothesis~(H2) provides the dynamical input:
on a surface with non-constant curvature, the frozen-mode entropy
$S(x) = \tfrac{1}{2}\sum_{m \ne m^*}\log\lambda_m(x)$
varies from point to point (points with larger $|R|$ have
smaller $\lambda_m$ and lower entropy).
The total entropy $S_{\mathrm{total}} = \int_S S(x)\,dA$
is maximised, subject to the constraint of fixed total energy
(the cosmological constant), when $C_1(x)$ is
uniform---i.e.\ when $R(x) = \mathrm{const}$.
This is the standard constrained-optimisation argument:
the Lagrange multiplier for the energy constraint is~$\Lambda$.

The derivation of~(H2) is therefore \emph{not} circular:
it does not assume constant curvature.
It uses the second law (entropy maximisation of the
polygon system, the Onsager selection principle of Paper~I)
on a \emph{general} background, and the maximum-entropy
configuration turns out to be constant curvature.
The Einstein equations are the unique equilibrium of
the polygon entropy.
