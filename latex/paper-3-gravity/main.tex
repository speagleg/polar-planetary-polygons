...
$N \ge 6$ (corrections $\le 1\%$).
At $N = 5$, the $9\%$ correction is the leading
...
\emph{spatial} Weyl tensor $W_{ijkl} = 0$ and
$R_{ij} = (R/d)\,g_{ij}$ on each time slice.
...
\paragraph{The Clausius relation and Newton's constant.}
At each palindromic threshold, the critical eigenvalue
$\lambda_{m^*} = 0$ creates a local Rindler horizon
with surface gravity $\kappa = V'(\rho^*) = \coth(\rho^*)$
and Unruh temperature $T = \kappa/(2\pi)$.
The Jacobson thermodynamic argument
\citep{Jacobson1995,Lashkari2014} gives the matter-coupled
Einstein equations from the Clausius relation
$\delta Q = T\,\delta S$ at every such horizon.
