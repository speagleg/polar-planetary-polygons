...
three conditions:
H1~(the $\csc^2$ uniqueness, a theorem),
H2~(entropy maximisation, combining a strict-concavity
theorem with the Onsager selection principle at negative
temperature), and
H3~(Hadamard universality, a theorem).
The mathematical content of~H2 is: the polygon entropy
functional $S_{\mathrm{poly}}[g]$ is strictly concave and
has a unique constrained maximum at $R(x) = \mathrm{const}$
(Jensen's inequality).
The physical content is the Onsager variational
principle: at negative temperature, the microcanonical
ensemble is dominated by the entropy-maximising macrostate.
...
One might object that the Onsager principle
...
One might object that the Regge Hamiltonian
$H = -\sum \kappa_i\kappa_j\,G_{g(z)}(z_i, z_j)$
is self-referential: both the circulations~$\kappa_v$
and the Green's function~$G_{g(z)}$ depend on the vertex
positions through the metric.
On a \emph{flat} background ($\Lambda = 0$), this
objection vanishes:
...
Hadamard expansion (Step~2 above) gives the correction
$\delta C_1 = -(N{-}1)R\varepsilon^2/4$, which is a
smooth $O(R\varepsilon^2)$ perturbation of the
flat-background Hamiltonian.
The Onsager prerequisites (compact phase space, bounded
energy, decreasing density of states at high energy)
are stable under smooth perturbations of the
Hamiltonian on a compact domain.
