...
\emph{Bridge: Schur's lemma on $\mathbb{Z}_N$ modes.}
The vortex uniqueness argument above shows that the
tangential eigenvalues $T_m$ are a polynomial of degree
$\le 2$ in~$m$, and $T_m = m(N{-}m)/2$ is the unique such
polynomial compatible with the Havelock sum rule
...
and by Schur's lemma any $\mathbb{Z}_N$-equivariant
scalar operator (including the quadratic Casimir of the
$\mathfrak{sl}(2,\mathbb{R})$ Laplace--Beltrami on
$\mathbf{H}^2$ restricted to $V_m$) is a multiple of the
identity, determined entirely by its value at a single
generator.
Its value at the polygon generator is fixed by the
Plancherel decomposition (\citealt{Helgason1984}, Ch.~III,
Thm~1.3) to be polynomial of degree $\le 2$ in the mode
label, and the Havelock sum rule forces this polynomial
to be $m(N{-}m)/2$.
Both sides therefore give the same $T_m = m(N{-}m)/2$,
...
\item \emph{Onsager contraction (constructive, finite~$N$).}
  ...
  at $N = 7$ (contraction factor $0.041$ via the sharp Sobolev bound,
  or $0.049$ via the coarser $L^\infty$ bound; explicit in both cases).
