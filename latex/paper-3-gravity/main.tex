...
In the semiclassical regime $k \gg 1$,
integrality corrections are~$O(1/k)$ and the CS formulation
is reliable \citep[\S3]{Witten1988}.
At $N = 7$ ($k \approx 8.6$), the semiclassical
approximation has $O(1/k) \approx 12\%$ corrections;
the integrality issue is an open limitation for
small~$N$ (see Paper~IV, \S\ref*{V-sec:gauge-group}).
