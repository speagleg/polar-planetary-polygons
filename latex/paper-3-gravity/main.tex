...
\item \emph{The coefficient is $c = 12\,b(N)$}
  (self-consistency, Proposition~\ref{prop:equiv-rr}):
  ...
  (Paper~I).
  The CL noise kernel produces a \emph{different} kinetic
  coefficient ($\sim N^2\rho^{*2}/((N{-}2)\coth^2\rho^*)$,
  which does not equal $12\,b(N)$ at any~$N$).
  The value $c = 12\,b(N)$ is instead the unique coefficient
  for which the WDW ground-state eigenvalue reproduces the
  classical Havelock spectrum---a \emph{uniqueness condition}
  from the eigenvalue structure, not a dynamical derivation
