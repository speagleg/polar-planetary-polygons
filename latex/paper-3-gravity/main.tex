...
H2~(Jaynes maximum entropy of the polygon partition function
at fixed energy), and
H3~(Hadamard universality).
The sole physical law invoked is energy conservation.
...
\item \textbf{(H2) is a theorem} (Jaynes maximum entropy):
  the polygon partition function
  $Z[g] = \sum_N e^{-b(N)}\prod_m|\lambda_m(g)|^{-1/2}$
  is a \emph{defined} functional of the metric~$g$
  (it counts polygon states, no physical identification needed).
  The entropy $S[g] = \log Z[g]$ is \emph{strictly concave}
  in~$R(x)$ (proved below).
  The Jaynes maximum-entropy principle
  \citep{Jaynes1957}---``the distribution maximising entropy
  subject to constraints is the least biased inference''---%
  gives: the metric maximising $S[g]$ at fixed total energy
  ($\int V\,dA = E$, energy conservation)
  has $R(x) = \mathrm{const}$.
  This is \emph{not} a physical hypothesis---it is a
  logical consequence of counting polygon states on a
  general background with a fixed energy constraint.
\end{itemize}
The theorem therefore has the structure:
\emph{three theorems $\Rightarrow$ Einstein equations.}
The sole physical law invoked is \textbf{energy conservation}
($\int V\,dA = E$).
The derivation does not assume gravity, thermodynamic
equilibrium, or any identification between mathematical
...
\item[\textup{(H2)}]
  ...
  the polygon partition function
  $Z[g] = \sum_N e^{-b(N)}\prod_m|\lambda_m(g)|^{-1/2}$
  is maximised over the metric at fixed total energy
  $\int V(\rho(x))\,dA = E$ (energy conservation).
  Proved by the Jaynes maximum-entropy principle
  \citep{Jaynes1957} applied to the
  strictly concave functional $\log Z[g]$
