...
\begin{itemize}
  \item \S\ref{sec:graviton}: the graviton at $N = 7$ and
    the Chern--Simons lift.
  \item \S\ref{sec:lichnerowicz}: the main theorem ---
    polygon entropy maximisation implies the Einstein equations.
  \item \S\ref{sec:spectral-bound}--\S\ref{sec:ope-identification}:
    spectral bound, holographic interpretation, and conformal
    block identification.
  \item \S\ref{sec:btz}--\S\ref{sec:wdw}: the polygon--BTZ
    phase transition and the Wheeler--DeWitt equation.
  \item \S\ref{sec:three-layer}--\S\ref{sec:triangle}:
    three-layer decomposition, higher-dimensional extension,
    and triangle universality.
  \item \S\ref{sec:gravity-partition}--\S\ref{sec:qm-gr-bridge}:
    partition function and Onsager--WDW correspondence.
\end{itemize}

\paragraph{Main result.}
The central theorem (Theorem~\ref{thm:polygon-einstein},
\S\ref{sec:lichnerowicz}) states:
...
The proof appears in \S\ref{sec:lichnerowicz}, immediately
following the graviton identification.
The remaining sections develop the supporting infrastructure
and consequences.
...
$dC_1/dR = -(N{-}1)\varepsilon^2/4$ (the Hadamard expansion),
...
\emph{Mechanism.}
The Havelock structure forces the interaction to be logarithmic;
the Hadamard parametrix then imprints the local curvature $R(x)$
on the polygon entropy; entropy maximisation (the Onsager principle)
selects $R = \mathrm{const}$, which is the vacuum Einstein equation.
...
(3b: Regge extension to smooth metrics; 3c: partition
...
This coefficient is universal: it follows from the
Hadamard--Minakshisundaram--Pleijel heat kernel expansion,
whose first Seeley--DeWitt coefficient $a_1 = R/6$ for the
scalar Laplacian in $2$D gives $R\varepsilon^2/4$ upon
angular averaging over the $N$-gon ring.
...
The $O(\varepsilon^4)$ Seeley--DeWitt correction ($\alpha = 1/48$)
is subdominant: the concavity-to-correction ratio
$(N{-}1)\sigma_2/(32\alpha\sigma_1) \ge 7.5$ for all $N \ge 7$
(see Code Availability), so strict concavity holds
throughout the small-ring regime.

\medskip
\noindent\emph{Step~3b (Extension to smooth metrics via Regge calculus).}
...
(stated before the theorem).

\noindent
\emph{Curved-background maximiser.}
...
the Hadamard expansion (Step~2) gives
...
preserves this maximiser \citep{DalMaso1993}.
...
\medskip
\noindent\emph{Step~3c (Partition function interpretation).}
...
Steps~1--3 are dimension-independent; the extension to
...
$\gamma \sim O(N)$.
It is uniquely fixed: $b(N)$ is the mode-independent
offset in $\lambda_m = b(N) - m(N{-}m)/2$, and any other
coefficient would make the WDW potential $m$-dependent.
