...
(Note: the $\mathbb{Z}_N$ Fourier mode $V_m$ is
one-dimensional, so no representation-theoretic structure
beyond the eigenvalue is needed; the identification
...
\item \emph{The coefficient $c = 12\,b(N)$ is derived from
  ...
  (The CL noise kernel gives a \emph{different} coefficient,
  $\sim N^2\rho^{*2}/((N{-}2)\coth^2\rho^*)$, which does not
  equal $12\,b(N)$ at any~$N$; the CL mechanism provides the
  kinetic \emph{term}, not its coefficient.)
...
\item \emph{Analytic}: the log-symbol Fourier coefficients $c_k$
  ...
  $T_m = 3, 5, 6, 6, 5, 3$):
  the log-symbol $\log T_m$ has Fourier coefficients
  $c_0 = 1.500$, $c_1 = 0.347$, $c_2 = 0.055$,
  $c_3 = 0$ (the discrete transform on
  $\mathbb{Z}_6$ terminates).
  The Szeg\H{o} integral
  $I_0 = \tfrac{1}{6}\sum_m \log T_m = 1.500$,
  the Szeg\H{o} constant
  $C_S = \exp(\sum k|c_k|^2) = 1.203$,
  and $S/c_1^2 = 0.46$.
