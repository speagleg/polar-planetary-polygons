...
\item \emph{Definition.}
  ...
  Define $c := 12\,b(N)$.
\item \emph{Holographic identification.}
  The vortex system on $\mathbf{H}^2$ is equivalent to
  $2{+}1$D gravity via the Green's function isomorphism
  (Paper~II).
  The Brown--Henneaux theorem then identifies $c$
  with the boundary Virasoro central charge:
  $c = 3\ell/(2G) = 6k$.
\item \emph{Consequence for the WDW equation.}
  The Feynman--Kac mapping of the statistical-mechanical
  path integral (\S\ref{sec:lichnerowicz}, Step~2b) produces
  a kinetic coefficient.
  This coefficient is $c$ because the breathing mode
  $\rho$ is the AdS radion, whose kinetic term is set
  by the Brown--Henneaux central charge (step~2).
...
step~1 (extraction from the eigenvalue data) and step~2
...
The kinetic coefficient $c = 12\,b(N)$ is the Brown--Henneaux
central charge (\S\ref{sec:intro-gravity}, logical architecture),
entering via the holographic identification of the breathing mode
with the AdS radion (Paper~IV, \S\ref*{V-sec:graviton}).
...
Quantum kinetic & $-\hbar^2/(2c)\;\Psi''$ &
  Brown--Henneaux central charge (\S\ref{sec:intro-gravity}) \\
...
equation~(\ref{eq:wdw}).
The kinetic coefficient $c = 12\,b(N)$ enters through the
chain: (i)~the three-layer decomposition extracts $b(N)$
as the unique mode-independent offset
(Proposition~\ref{prop:equiv-rr});
(ii)~the vortex--gravity isomorphism (Paper~II) identifies
the vortex system with $2{+}1$D gravity;
(iii)~the Brown--Henneaux theorem gives
$c = 3\ell/(2G) = 12\,b(N)$ as the boundary Virasoro central
charge (\S\ref{sec:intro-gravity}, logical architecture);
(iv)~the breathing mode~$\rho$ is the AdS radion, whose
kinetic term in the dimensionally-reduced action is
$S = \int(c/2)\dot\rho^2\,d\tau$.
