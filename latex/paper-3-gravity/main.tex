...
all numerical predictions in Paper~IV use the exact~$c$.
...
\begin{equation}\label{eq:einstein-from-polygon}
  R_{\mu\nu} = \Lambda\,g_{\mu\nu},
  \qquad
  \Lambda = \frac{2\bigl[f(m^*,N) - (N{-}1)\bigr]}
                 {(N{-}1)\,\varepsilon^2}.
\end{equation}
...
\medskip
\noindent\textbf{Step~5. The equivalence principle forces $R = \mathrm{const}$.}
...
$N \le 6$: $f(m^*) < N{-}1$, so $\Lambda < 0$ (AdS);
$N = 7$: $f(3,7) = 6 = N{-}1$, so $\Lambda = 0$ (flat);
$N \ge 8$: $f(m^*) > N{-}1$, so $\Lambda > 0$ (dS).
