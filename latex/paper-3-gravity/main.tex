...
The breathing mode~$\rho$ parametrises the one-parameter family
$z_k(\rho) = \rho\,e^{2\pi i k/N}$ of regular polygons.
The effective second-order WDW equation for~$\rho$ does
\emph{not} arise from a symplectic reduction of the
Kirchhoff system (the cotangent sum
$\sum_{p}\csc^2(\pi p/N) = (N^2{-}1)/3$ is purely polynomial and
does not equal $12\,b(N)$, which contains transcendental terms).
Instead, $c = 12\,b(N)$ enters as the \emph{Brown--Henneaux
central charge} of the boundary CFT (\S\ref{sec:holographic}),
through the holographic identification of the breathing mode
with the radion of the Seifert fibration (Paper~IV,
\S\ref*{V-sec:graviton}).
The WDW equation is therefore a \emph{gravitational} effective
description, valid in the regime where the vortex-gravity
isomorphism holds, rather than a direct quantisation of the
