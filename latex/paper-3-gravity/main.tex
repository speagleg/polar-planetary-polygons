...
The Laplace--Beltrami operator on $\mathbf{H}^2$
restricted to $\mathbb{Z}_N$-equivariant functions
is the quadratic Casimir
$\Omega_{\mathfrak{sl}(2,\mathbb{R})}$ on the
discrete series $D_j^+$ with $j = m^*/2$
(\citealt{Helgason1984}, Ch.~III, Thm~1.3:
multiplicity-free Plancherel).
By Helgason symmetric-space theorem,
$\Omega|_{D_j^+} = j(j{-}1)$; translating to the
vortex mode label via $j = (m + \text{const})/2$
yields $m(N{-}m)/2$.
On the $1$-dimensional irreducible $V_m$
(Schur: a linear map $V_m \to V_m$ is a scalar),
the quadratic Casimir determines the operator uniquely.
Both the vortex side and the gravity side therefore give
the same polynomial $T_m = m(N{-}m)/2$, and the identification
is forced by Steps~(i), (ii), (iii), together with the
...
\item \emph{Onsager contraction (constructive, finite~$N$).}
  ...
  at $N = 7$ (contraction factor $0.041$ via Sobolev bound,
  or $0.049$ via cruder Hoeffding bound; explicit in both cases).
...
The value $c = 12\,b(N)$ is overdetermined:
three routes through the Havelock eigenvalue data
give the same answer
(a non-trivial consistency check on the algebra,
though not on independent data):
(i)~the quantum BO factorization requires $c$ to be
mode-independent (the Havelock eigenvalues $\lambda_m = C_1 - T_m$
have mode-independent $C_1$, so the one-loop effective action
$-\log Z_{\mathrm{angular}} = -\tfrac{1}{2}\sum\log\lambda_m
= -(N{-}2)\log C_1 + f(C_1)$ depends on $\rho$ only through $C_1(\rho)$,
giving $c = (N{-}1) \cdot (\partial C_1/\partial\rho)^2 / C_1$
evaluated at the saddle, this equals $12\,b(N)$ by
the Gauss product formula $\prod\sin(\pi p/N) = N/2^{N-1}$);
(ii)~Brown--Henneaux gives $c = 3\ell/(2G)$ with
$\ell$ and $G$ from the Seifert geometry;
(iii)~the Polyakov formula relates $c$ to the conformal anomaly
of the boundary CFT, which equals $12\,b(N)$ by the
eigenvalue decomposition (Proposition~\ref{prop:equiv-rr}).)
