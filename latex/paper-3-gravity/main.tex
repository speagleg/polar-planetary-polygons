...
$\delta R/\delta\varphi = -2R$ and
$\delta(dA)/\delta\varphi = 2\,dA$.
...
\[
  ...
  + 2\,S(x).
\]
At the entropy maximum this is constant for all~$x$, giving
$F(R(x)) = \Lambda_\varphi$ with $F$ strictly monotone
($dS/dC_1 > 0$, $dC_1/dR < 0$).
...
The Seeley--DeWitt coefficient $a_1 = R/6$ controls the
curvature dependence of the diagonal heat kernel
$K(x,x;t) = (4\pi t)^{-1}(1 + Rt/6 + \cdots)$;
the angular average over the $N$-gon chord geometry
converts $1/6$ to $1/4$, with the additional $R\varepsilon^2/12$
arising from the metric distortion of chord distances on
the geodesic circle: $d_p^2 = 4\varepsilon^2\sin^2(\pi p/N)
(1 - R\varepsilon^2/6 + O(\varepsilon^4))$.
\end{proof}
...
\noindent
\emph{Strict concavity at all orders.}
The second functional derivative of $S_\varepsilon[R]$ with respect
to $R(x)$ has two contributions:
\[
  \frac{\delta^2 S_\varepsilon}{\delta R(x)^2}
  \;=\; \underbrace{S''(C_1)\!\left(\frac{\partial C_1}{\partial R}\right)^{\!2}}_{\text{concavity (negative)}}
  \;+\; \underbrace{S'(C_1)\,\frac{\partial^2 C_1}{\partial R^2}}_{\text{correction}}.
\]
From the Hadamard expansion,
$\partial C_1/\partial R = -(N{-}1)\varepsilon^2/4 + O(\varepsilon^4)$
and
$\partial^2 C_1/\partial R^2 = 2(N{-}1)\alpha\,\varepsilon^4$
with $\alpha = 1/48$ (the angular-averaged second
Seeley--DeWitt coefficient).
The concavity term contributes
$S'' \times (N{-}1)^2\varepsilon^4/16$;
the correction contributes
$S' \times 2(N{-}1)\alpha\,\varepsilon^4$.
Both are $O(\varepsilon^4)$, so $\varepsilon$ cancels from
their ratio:
\begin{equation}\label{eq:concavity-ratio}
  \frac{|S''|\,(N{-}1)}{32\,\alpha\,S'}
...
Since the ratio is $\varepsilon$-independent, the concavity
term dominates the correction at every~$\varepsilon$
in the stability range.
Strict concavity of $S_\varepsilon[R]$ holds exactly.
...
\emph{exact}: the Havelock
eigenvectors are $\rho$-independent
(Lemma~\ref{lem:bo-exact}), so
...
geometry reduces to the Havelock Hessian by the
following argument.
...
with this $\mathbb{Z}_N$ action.
By Schur's lemma, $\Delta_L$ decomposes into
$\mathbb{Z}_N$ irreducible representations:
each perturbation mode carries a definite
$\mathbb{Z}_N$ charge $m = 0, \ldots, N{-}1$.
On the $m$-th sector, the angular part of $\Delta_L$
is a circulant (determined by the $\mathbb{Z}_N$
Fourier coefficient of the Green's function Hessian),
whose eigenvalue is $\lambda_m = C_1 - m(N{-}m)/2$
...
\item \emph{Existence (Caldeira--Leggett).}
  ...
  because the Gaussian truncation does not preserve the exact
  mode-by-mode factorisation of the integrable bath.
