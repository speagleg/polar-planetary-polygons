...
all three hypotheses are proved or derived, with no circularity.
...
For $N > 4$: the moduli space $\mathcal{M}_{0,N+1}$ has
dimension~$2(N{-}3)$, but the $\mathbb{Z}_N$ symmetry constrains
the invariant subspace $\mathcal{M}_{0,N+1}^{\mathbb{Z}_N}$
to dimension~$2$ (the ring radius and overall phase).
This $2$-dimensional moduli space admits a single
$\mathbb{Z}_N$-invariant saddle: the symmetric conical metric.
The reduction is equivalent to the $\mathbb{Z}_N$ orbifold
construction: the $N$-point function with $\mathbb{Z}_N$ symmetry
equals the $1$-point function of the twist field~$\sigma_m$ on
$\mathbb{C}/\mathbb{Z}_N$, which has no competing OPE channel
(a $1$-point function has a unique vacuum contribution).
...
\begin{equation}\label{eq:wdw}
  ...
  + \delta_m\biggr]\Psi = 0,
\end{equation}
with $c = 12\,b(N) \approx N^2$ and $\hbar = 1$ (so the kinetic coefficient
$\hbar^2/(2c) = 1/(2c)$; the semiclassical limit is $c \to \infty$).
...
$R_{\max} = (N^2{+}2N{-}6)/(4N^2) < 1$ for all $N \ge 3$,
so the non-crossing conclusion is unaffected.
...
\item[\textup{(H3)}]
  \emph{Equivalence principle}: conditions \textup{(H1)--(H2)} hold
  at every point~$x \in S$ (the laws of vortex dynamics are
...
Canonical quantisation with Planck mass $c = 12\,b(N) \approx N^2$ gives the
Wheeler--DeWitt equation~(\ref{eq:wdw}).
...
it states that the polygon stability structure
\textup{(H1)--(H2)} is location-independent.
...
On a space with nonzero Weyl tensor, the Green's function acquires
angular dependence (an $\ell = 2$ quadrupole from $W_{ijkl}$
at leading order).
This angular dependence makes $C_1$ depend on~$m$, violating~(H1).
Therefore (H1) at every point~$x$ forces the Green's function
to be isotropic at every point, i.e.\ the spatial geometry is
\emph{maximally symmetric}: $R_{ij} = (R/d)\,g_{ij}$ with $W_{ijkl} = 0$.
...
follows from the Lashkari--McDermott--Van~Raamsdonk (2014)
