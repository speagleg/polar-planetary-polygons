...
\begin{proof}
\emph{Vortex uniqueness (Paper~I, Remark~\ref*{I-rmk:polynomial-uniqueness}).}
The $\mathbb{Z}_N$ Fourier sum
$S_\alpha(m) = \sum_p [1{-}\cos(2\pi pm/N)]/|\sin(\pi p/N)|^\alpha$
is polynomial in~$m$ if and only if $\alpha = 2$.
At $\alpha = 2$, the holomorphic factorization
$|\sum\omega^{pk}|^2$ (a polynomial in the root of unity)
produces $T_m = m(N{-}m)/2$.
For $\alpha \ne 2$, the second finite difference $\Delta^2 S_\alpha$
has infinite Fourier support (Paley--Wiener), precluding
polynomial dependence on~$m$.
The kernel $\csc^2$ corresponds to the $2$D logarithmic
Green's function ($d = 2 \Leftrightarrow \alpha = 2$);
no other spatial dimension gives polynomial eigenvalues.

\emph{Gravity uniqueness.}
The $2$D base~$\mathbf{H}^2$ has isometry group
$\mathrm{SL}(2,\mathbb{R})$, which is \emph{rank~$1$}:
the symmetric space $G/K = \mathrm{SL}(2,\mathbb{R})/\mathrm{SO}(2)$
has one angular coordinate, so $\mathbb{Z}_N$ modes are
labelled by a single integer~$m$.
For rank~$\ge 2$ groups ($G/K$ has $\ge 2$ angular coordinates),
the $\mathbb{Z}_N$ decomposition involves multiple quantum
numbers and the Casimir is not a polynomial in a single
variable~$m$.
Among rank-$1$ symmetric spaces, $\mathbf{H}^2$ is uniquely
two-dimensional; the unique first-order topological gauge
theory with isometry group $\mathrm{SL}(2,\mathbb{R})$
is CS theory (\citealt{Witten1988}).
The CMS Casimir on the $\mathbb{Z}_N$ mode is then
$m(N{-}m)/2$ (Helgason + Havelock): a degree-$2$
polynomial in the single quantum number~$m$,
with the specific form $m(N{-}m)/2$ forced by
the $\csc^2$ angular kernel of the $2$D Laplacian.

\emph{Bridge.}
Both sides give the same polynomial $T_m = m(N{-}m)/2$.
On the $1$-dimensional $\mathbb{Z}_N$ irreducible~$V_m$
($m \ne N{-}m$), the quadratic Casimir determines the
operator uniquely (Schur: a linear map $V_m \to V_m$
on a $1$D space is a scalar).
The identification is therefore not a choice among
alternatives but the unique match between the
$2$D polynomial structure and the $2{+}1$D topological
structure.
\end{proof}
...
The uniformized metric ($R = R_0 = \mathrm{const}$,
...
microcanonical entropy; this uses Lemma~\ref{VI-lem:nonadiabatic}
from Paper~VI).
...
where $\gamma_\varepsilon$ is the regularized Robin
...
For the regularized Robin function
...
because $\varepsilon$ is the Cauchy blob width
(Paper~II, Proposition~\ref*{II-prop:blob}), a free
regularization parameter independent of the polygon
radius~$\rho^*$.  Choosing $\varepsilon < (C_S C_E')^{-1/2}$
(which is always possible since $\varepsilon \to 0$ is the
point-vortex limit) gives $\kappa < 1$.
...
This triangulation is imposed by hand as a regularization.
