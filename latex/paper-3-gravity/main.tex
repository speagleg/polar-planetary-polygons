...
\medskip
\noindent\textbf{Step~2b. The WDW equation from integrating out the frozen modes.}
The $N$-gon has $N{-}2$ angular modes (frozen) and one breathing
mode~$\rho$.
The full partition function is
$Z = \int d\rho\,\prod_{m \ne m^*}da_m\;
\exp\!\bigl(-\beta[V(\rho) + \tfrac{1}{2}\sum\lambda_m(\rho)|a_m|^2]\bigr)$.
Integrating out the angular modes (Gaussian) gives the
effective action for~$\rho$:
$S_{\mathrm{E}}[\rho] = \int_0^\beta[\tfrac{c}{2}\dot\rho^2
+ V_{\mathrm{eff}}(\rho)]\,d\tau$,
where the kinetic coefficient~$c$ receives two contributions:
(i)~the \emph{naive mass} $c_0 = N$
(from the configuration-space metric $\sum|dz_k|^2 = N\,d\rho^2$),
and
(ii)~the \emph{one-loop enhancement} from the $\rho$-dependence
of the frozen-mode determinant
$\prod_{m \ne m^*}|\lambda_m(\rho)|^{-1/2}$.
The total effective mass is $c = 12\,b(N)$:
this equals the equivariant index
(Proposition~\ref{prop:equiv-rr}), which is topological
and therefore $\rho$-independent.
The one-loop enhancement factor is $c/N \approx N$
(each of the $N{-}1$ frozen modes contributes ${\sim}\,N$ to
the effective mass through its coupling to~$\rho$),
giving $c \approx N^2$ at leading order.

The transfer matrix $T = e^{-\epsilon\,H_{\mathrm{WDW}}}$
of this effective path integral satisfies
