...
The point vortex system on a constant-curvature surface
is the unique $2$D Hamiltonian system whose tangential
eigenvalues are polynomial in the mode number~$m$.
...
\item \emph{Onsager contraction (constructive, finite~$N$).}
  ...
  or $0.068$ via the coarser $L^\infty$ bound; explicit in both cases).
...
\item[\textup{(H1)}]
  ...
  Proved on constant-curvature surfaces by
  Theorem~\ref*{I-thm:riemannian-havelock}
  (the Robin function contributes zero to the tangential
  eigenvalues).
  On a general surface, (H1) holds to leading order
  in~$\varepsilon$ via the Hadamard parametrix:
  $C_1 = (N{-}1)[1 + O(\varepsilon^2)]$ with the
  $O(\varepsilon^2)$ correction mode-independent
  at fixed~$x$.
\item[\textup{(H2)}]
...
the cruder basin estimate $\mathrm{Var} \le 2$ gives $0.068$).
...
$(1/6)^2 \times 1.40 \times 1.75 = 0.068 < 1$.
...
$(1/6)^2 \times e^{0.30} \times 1.48
= (1/6)^2 \times 1.35 \times 1.48 = 0.056 < 1$.
