...
The orbifold CFT at central charge $c = 12N^2$ identifies the Havelock
...
The main result: the Havelock eigenvalue structure, applied via test polygons
at every point of a spatial slice, implies the vacuum Einstein equations
$R_{\mu\nu} = \Lambda g_{\mu\nu}$ in any dimension---with the Clausius relation
giving a universal ratio~$4\pi$ that determines the Newton constant and the
matter coupling.
\end{abstract}
...
equals $j(j{+}1)$ for an integer~$j \ge 2$.
The spin is $j = 2$: the graviton.
\end{proposition}
...
For $k \ge 4$: $j$ is irrational (the equation
$2j^2 + 2j = k^2 + k$ has no further integer solutions
by the theory of Pell equations on the quadratic
$(2j{+}1)^2 - 2(2k{+}1)^2 = -1$).
\end{proof}
...
\end{equation}
where $c = N^2$ is the orbifold central charge
...
We now present the main result of the gravitational theory:
a derivation of the vacuum Einstein equations in $2{+}1$
dimensions from the Havelock eigenvalue structure.
The derivation has six steps, each proved in the preceding sections.
We state the theorem first, then collect the proof.

\begin{theorem}[Polygon stability implies the Einstein equations in $2{+}1$D]
...
\item[\textup{(H3)}]
  \emph{Universality}: conditions \textup{(H1)--(H2)} hold
...
Canonical quantisation with Planck mass $c = 12N^2$ gives the
...
\medskip
\noindent\textbf{Step~5. The test polygon principle forces $R = \mathrm{const}$.}
...
\begin{remark}[The derivation in $d \ge 3$: equivalence, not obstruction]
\label{rmk:higher-d}
...
\paragraph{Supporting evidence: the Clausius relation.}
...
The ratio $4\pi$ is \emph{universal} across all thresholds
$N = 7, 8, 9, \ldots$ (verified to~$10^{-10}$ for $N \le 20$).
In the Jacobson framework \citep{Jacobson1995},
universality of $\delta Q = T\delta S$ across local Rindler horizons
