...
three conditions, of which \emph{only one is a physical hypothesis}:
H1~(the $\csc^2$ uniqueness: a \emph{theorem}, not an assumption)
and H3~(Hadamard universality: also a \emph{theorem}) are automatic
on any Riemannian surface;
the sole physical input is H2~(entropy maximisation, derived from
the Onsager principle via the vortex-gravity isomorphism).
...
\textbf{Logical status of the three hypotheses.}
Of the three conditions below, \textbf{only (H2) is a
physical hypothesis}.
Conditions (H1) and (H3) are \emph{mathematical theorems},
not assumptions:
\begin{itemize}
...
\item \textbf{(H2) is the sole physical input}:
  the polygon system reaches thermodynamic equilibrium.
  This is derived (not assumed) from the Onsager selection
  principle via the vortex-gravity isomorphism
  (\S\ref{sec:einstein-derivation} below).
\end{itemize}
The theorem therefore has the structure:
\emph{two automatic theorems $+$ one derived physical input
$\Rightarrow$ Einstein equations.}
The ``derivation'' does not assume gravity---it derives
$R = \mathrm{const}$ from the universal $\csc^2$ structure
of the Green's function (a mathematical fact) combined with
entropy maximisation (a physical principle).
}}
\medskip
...
\item[\textup{(H2)}]
  \emph{Entropy equilibrium} \textbf{[the sole physical input]}:
  the frozen-mode entropy
  $S(x) = \tfrac{1}{2}\sum_{m \ne m^*}\log\lambda_m(x,\varepsilon)$
  is maximised over the surface --- i.e., the polygon system
  is in thermodynamic equilibrium.
  Derived from the Onsager selection principle
  via the vortex-gravity isomorphism
