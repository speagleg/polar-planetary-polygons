...
for $\mathrm{SL}(2,\mathbb{R})$ the analytic continuation
in~$k$ is controlled by the Plancherel measure
of the continuous series \citep{Witten1991};
...
\item[\textup{(H2)}]
  ...
  ($d^2S/dC_1^2 < 0$; proof of Theorem~\ref{thm:polygon-einstein}).
...
(a standard sub-Gaussian inequality;
see \citealt{BoucheronLugosiMassart2013}, Theorem~2.1;
on a compact surface, $\delta R$ is bounded;
$\mathrm{Var}(R) \le 2$ and Chebyshev give
$|\delta R| \le \sqrt{2/\epsilon}$ on a set of
measure $\ge 1 {-} \epsilon$):
\[
  \mathrm{Var}(R') \;\le\;
...
On the Bolza surface ($\mathrm{Vol} = 4\pi$), the
Sobolev embedding $H^1(\Sigma) \hookrightarrow L^\infty(\Sigma)$
(valid for compact 2-manifolds; \citealt{Aubin1998},
Theorem~2.21) gives
$\|R - R_0\|_{L^\infty} \le C_S\,\|R - R_0\|_{H^1}$
with $C_S$ depending on the genus and area.
For $\mathrm{Var}(R) \le 2$ and $\|\nabla R\|_{L^2}^2 \le 2$
(both satisfied by metrics near the uniformised one):
$\|R - R_0\|_{H^1}^2 \le 4$, giving $\|R - R_0\|_\infty \le 2C_S$.
The Poincar\'e-uniformised metric ($R = \mathrm{const}$) has
$\mathrm{Var} = 0$, and the basin $\mathrm{Var} \le 2$
contains a metrically thick $H^1$ neighbourhood of this metric.
...
keeps the metric within this domain: $R = \mathrm{const}$ implies
$\lambda_m = C_1 - f(m) > 0$ for all $m \ne m^*$,
so the Jensen argument operates entirely within the concavity domain.
...
symmetry of the metric; the Lipschitz regularity of the
orbifold boundary suffices for the FG scattering operator,
\citealt{GrahamZworski2003}).
