...
(Witten 1988), the vortex polygon is a Wilson line
whose eigenvalue is the $\mathrm{SL}(2,\mathbb{R})$ Casimir
$C_2(R) = j(j{+}1)$ of the representation~$R$
(Paper~II).
The Havelock Casimir $f(m,N) = m(N{-}m)/2$ IS this
$\mathrm{SL}(2,\mathbb{R})$ Casimir---not by coincidence,
but because the vortex eigenvalue structure IS the
CS Wilson line spectrum.
...
Level quantisation requires integer~$k$ only for
\emph{compact} groups ($\pi_3(G) = \mathbb{Z}$);
for $\mathrm{SL}(2,\mathbb{R})$,
$\pi_3 = 0$ and no topological obstruction exists
\citep[\S3]{Witten1988}.
...
The vortex spectral data (the Havelock eigenvalue sequence)
is linked to the CS partition function through the
Stieltjes--Wigert orthogonal polynomial structure of
\citet{Tierz2006}.
...
$Z \sim |\rho{-}\rho^*|^{-1}$.
$Z_{\mathrm{frozen}}$ for odd~$N$ is computed
from the remaining Casimir gaps
...
\emph{Notation: two forms of $C_1$.}
The WDW potential uses the \emph{Green's function offset}
$C_1^{\mathrm{GF}}(\rho) = \log(2\sinh\rho) + b(N)$,
which diverges as $\rho \to 0$.
The holographic sections use the \emph{constrained angular stiffness}
$C_1(\xi) = (N{-}1)(1{+}\xi^2)/(1{-}\xi)^2$ (eq.~\ref{eq:C1-H2}),
...
\item \emph{The coefficient is $c = 12\,b(N)$}
  (equivariant index, Proposition~\ref{prop:equiv-rr}):
  $b(N)$ is the unique mode-independent offset in the
  eigenvalue decomposition, computed from the $\csc^2$
  kernel to $10^{-16}$ precision.
  This is \emph{not} the CL friction $\gamma \sim O(N)$;
  it is a topological invariant of the eigenvalue structure
  ($c \sim N^2$).
  No parameter is fitted: $b(N)$ is determined by the kernel.
\end{enumerate}
...
(Paper~IV, Theorem~\ref*{V-thm:zero-anomaly}).
...
\emph{Why the variational principle is global.}
The entropy $S(x)$ at each point~$x$ is a functional of
the \emph{metric}~$g$: it depends on~$g$ through the
Green's function $G(x,y)$ and the Hadamard expansion.
The total entropy $S_{\mathrm{total}} = \int S(x)\,dA$
is therefore a functional of the single global object~$g$.
Maximising $S_{\mathrm{total}}$ over the space of Riemannian
metrics (at fixed topology and fixed total polygon energy)
is a standard calculus-of-variations problem.
No ``communication between polygons'' is needed: the metric
constrains all points simultaneously because it is one object.
(This is the same logic as Jacobson 1995: different Rindler
horizons do not communicate, but the metric satisfies all
local constraints simultaneously.)
The Euler--Lagrange equation of the variational problem
$\delta S_{\mathrm{total}} / \delta g_{ij}
= \Lambda\;\delta E_{\mathrm{total}} / \delta g_{ij}$
reduces to $R = \mathrm{const}$ by the concavity of~$S$
and the Hadamard expansion (Steps~2--3 above).
