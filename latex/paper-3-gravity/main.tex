...
Consider the family of radial kernels $h_\alpha(r)
= -r^{2-\alpha}/(2-\alpha)$ for $\alpha \ne 2$ and
$h_2(r) = -\ln r$, which generate the Fourier sums
\[
  S_\alpha(m)
  = \sum_{p=1}^{N-1}
    \frac{1 - \cos(2\pi pm/N)}{\bigl(2\sin(\pi p/N)\bigr)^\alpha}.
\]
We show that $S_\alpha(m)$ is a polynomial in~$m$ of degree
$\le 2$ if and only if $\alpha = 2$.
The second forward difference of a cosine is
\[
  \Delta^2\!\cos(2\pi pm/N)
  = 2\cos(2\pi pm/N)\bigl[\cos(2\pi p/N) - 1\bigr]
  = -4\sin^2(\pi p/N)\cos(2\pi pm/N),
\]
so, substituting and cancelling,
\begin{equation}\label{eq:delta2-Salpha}
  \Delta^2 S_\alpha(m)
  = 2^{2-\alpha}\sum_{p=1}^{N-1}
    \sin^{2-\alpha}(\pi p/N)\,\cos(2\pi pm/N).
\end{equation}
$S_\alpha$ is a polynomial of degree $\le 2$ in~$m$
if and only if $\Delta^2 S_\alpha(m)$ is independent of~$m$.
The right-hand side of~\eqref{eq:delta2-Salpha} is a
discrete cosine transform of the sequence
$c_p = \sin^{2-\alpha}(\pi p/N)$, $p = 1, \ldots, N{-}1$.
Since the discrete cosine transform is an invertible
linear map on this index set, the transform is constant
in~$m$ if and only if $c_p$ is \emph{constant in~$p$}.
But $\sin(\pi p/N)$ takes distinct values for distinct
$p \in \{1, \ldots, \lfloor N/2 \rfloor\}$, so
$c_p = \sin^{2-\alpha}(\pi p/N)$ is constant in~$p$
if and only if the exponent vanishes, i.e.\ $\alpha = 2$.
At $\alpha = 2$, $c_p \equiv 1$, the transform reduces to
$\sum_{p=1}^{N-1}\cos(2\pi pm/N) = -1$, and
$\Delta^2 S_2(m) = -1$, consistent with the Havelock
identity $S_2(m) = m(N{-}m)/2$ (Paper~I,
Lemma~\ref*{I-lem:havelock}).
Since $\alpha = 2$ corresponds precisely to the $2$D
logarithmic Green's function
($-\nabla^2 h_2 = 2\pi\delta$ in $\mathbb{R}^2$),
no other spatial dimension produces polynomial
...
circulant; \citealt{Helgason1984}, Ch.~III).
...
Kiessling (\citeyear{Kiessling1993}, Theorem~2.1) proves
...
(\citealt{Rockafellar1970}, Cor.~25.5.2:
