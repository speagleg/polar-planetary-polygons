...
\]
with $c = 12\,b(N)$ extracted from the eigenvalue
decomposition (Proposition~\ref{prop:equiv-rr}).
The second-order kinetic term arises from the
Caldeira--Leggett mechanism: the $N{-}2$ angular modes
form a bath for the collective coordinate~$\rho$,
producing an Onsager--Machlup noise kernel
...
The Caldeira--Leggett conditions are satisfied at
leading order: (i)~the angular modes are harmonic
(exact Fourier diagonalisation); (ii)~the coupling is
mode-independent ($C_1' = \coth\rho$ for all~$m$);
(iii)~timescale separation holds
($\tau_{\mathrm{tunnel}} \sim e^{S_{\mathrm{BO}}} \gg 1$).
The bath has $N{-}2$ discrete modes (not a continuum),
so the Caldeira--Leggett derivation is a controlled
approximation at large~$c$ (where $N{-}2$ modes
approximate a dense spectral density) rather than an
exact result.
\end{proposition}
