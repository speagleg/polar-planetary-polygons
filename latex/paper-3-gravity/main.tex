...
H2~(thermalization of the integrable angular bath to the
generalised Gibbs ensemble), and
H3~(Hadamard universality).
The physical content of~H2 is:
``$N{-}2$ coupled harmonic oscillators thermalize''%
---a consequence of the CMS integrability of the Havelock system.
...
\item \textbf{(H2) is a theorem}
  (thermalization of the integrable angular bath):
  ...
  Integrable systems thermalize to the
  \emph{generalised Gibbs ensemble} (GGE):
  the maximum-entropy state subject to all conserved
  quantities \citep{Rigol2007}.
  The conserved quantities of the CMS system are the
  Casimirs $f_m^k$ ($k = 1, 2, \ldots$) from the Magri
  bihamiltonian hierarchy (\S\ref{sec:three-layer}).
  The GGE equilibrium of the angular bath, combined with
  the Caldeira--Leggett mechanism (\S\ref{sec:wdw}),
  drives the breathing mode~$\rho$ to the state that
  maximises $S[g] = \log Z[g]$
  ...
  \emph{physical consequence} of the thermalization of
  an integrable system, which is a theorem of statistical
  mechanics \citep{Rigol2007}.
\end{itemize}
The theorem therefore has the structure:
\emph{three theorems $\Rightarrow$ Einstein equations.}
The physical content of~(H2) is:
\textbf{``$N{-}2$ coupled harmonic oscillators thermalize''}%
---a consequence of the integrability of the CMS/Havelock
system, not an inference principle or a physical hypothesis.
}}
\medskip
...
\item[\textup{(H2)}]
  ...
  the $N{-}2$ angular Havelock modes form an integrable
  (CMS) harmonic bath that thermalises to the generalised
  Gibbs ensemble \citep{Rigol2007}.
  The GGE equilibrium maximises
  $S[g] = \log Z[g]$ (the one-loop effective action)
  subject to fixed total energy
  $\int V(\rho(x))\,dA = E$
  and the CMS conserved quantities.
...
The Jaynes derivation above is self-contained
(it uses only the partition function and energy conservation).
