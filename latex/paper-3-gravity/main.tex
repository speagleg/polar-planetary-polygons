...
\item \emph{Consistency check (Brown--Henneaux).}
  \emph{Define} $G := 3\ell/(2c)$ (with $\ell = 1$).
  The Brown--Henneaux relation $c = 3\ell/(2G)$ is then
  a tautology ($c = c$); $G$ is a \emph{derived quantity},
  not independently determined.
  The consistency content is that a \emph{single} $c$
  extracted from the eigenvalue data simultaneously
  serves as the Virasoro central charge, the CS level
  ($k = c/6$), and Newton's constant ($G = 3/(2c)$).
  This identification is a \emph{consequence}, not an input.
...
the \emph{same} $\mathfrak{sl}(2,\mathbb{R})$ that
...
\emph{same} $\mathfrak{sl}(2,\mathbb{R})$: the isometry algebra
of~$\mathbf{H}^2$.
\end{proposition}
...
(\texttt{proofs/casimir\_equals\_havelock.py},
verified to $10^{-15}$ for $N = 3,\ldots,12$
...
and the CS gauge group acts on the \emph{same}
...
\item \emph{OPE coefficient bound (Liouville action):}
  ...
  \]
  (\texttt{proofs/identity\_block\_dominance.py},
  verified $N = 3,\ldots,16$):
  $11\%$ at $N = 5$,
  $1.7\%$ at $N = 6$,
  $0.14\%$ at $N = 7$,
  $\sim 10^{-10}$ for $N = 11$.
...
(\texttt{proofs/spectral\_edge\_transition.py}, verified
$N = 7,\ldots,30$), giving $\rho(\lambda) = (2\lambda)^{-1/2}$
...
\emph{computed}, not fitted: it is the unique mode-independent
...
\item \emph{Exact coefficient (integrability).}
  ...
  \emph{exact}---higher loops vanish identically.
...
determines $c = 12\,b(N)$ exactly.
No Gaussian approximation enters the final result.

\emph{Four-role consistency.}
The coefficient $c = 12\,b(N)$ (computed from the vortex
spectrum alone) then plays four roles:
\begin{enumerate}
\item \emph{WDW kinetic coefficient}: $c$ appears in
  $-1/(2c)\,d^2/d\rho^2$;
\item \emph{CS level}: $k = c/6 = 2b(N)$;
\item \emph{Brown--Henneaux}: $G := 3\ell/(2c)$
  (a \emph{definition}, verified by $c = 3\ell/(2G)$);
\item \emph{Orbifold CFT}: the ground-state energy
  $E_0(m) = -c/(24N) + cm(N{-}m)/(24N^2)$
  reproduces $\lambda_m$ for all~$m$
  (Proposition~\ref{prop:orbifold-havelock}).
\end{enumerate}
These are four consequences of the same algebraic structure,
not four independent determinations.
If any role required a different value, the framework would
be inconsistent; their agreement on $c = 12\,b(N)$ is a
non-trivial self-consistency check.
The identification $c_{\mathrm{Havelock}} = c_{\mathrm{BH}}$
is a physical assumption---the vortex--gravity isomorphism
that underlies the entire framework---tested by this
four-role agreement.
...
($E = -\sum_{p<q}\ln\sin(\pi p/N)$ is the polygon energy),
$R_0 = 2\pi\chi/\mathrm{Vol}$ is the mean curvature, and
...
vortices with mean-field density
$\rho(x) \propto \exp(V(x)/|T|)$,
where $V(x) = V(C_1(R(x)))$ is the BO potential evaluated
at the local curvature~$R(x)$
(through the Hadamard expansion of~$C_1$).
...
This is by design: (H1) and~(H3) provide the mathematical
arena (logarithmic interaction, Hadamard universality);
(H2) provides the physics (the Onsager variational principle
selects the entropy-maximising geometry from that arena).
The entire gravitational content of the theorem resides
in~(H2).
Objecting that ``(H2) does all the work'' is equivalent
to noting that the physical hypothesis has physical content.
...
\emph{exact}, not a Gaussian approximation: the CMS angular modes
are integrable (Liouville--Arnold, Paper~III \S\ref{sec:three-layer}),
...
$V = \log(2\sinh\rho) + b(N) - f_{m^*}$ is a \emph{monotone}
...
at the threshold.
This ratio is \emph{computed} for all $N = 7, \ldots, 24$
(\texttt{proofs/jensen\_epsilon4\_bound.py}):
...
(quantitative refinement---not a logical prerequisite for the
proof; may be skipped without loss of continuity).}
...
(\texttt{proofs/entropy\_regularization.py};
...
$< 10^{-7}$ for $N \ge 15$
(\texttt{proofs/clausius\_cardy\_rt.py}, verified $N = 7,\ldots,20$).
