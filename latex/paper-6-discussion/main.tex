...
and six structural identifications forced by the geometry,
...
$m_s/m_b = 0.045$ & Derivation$^\dagger$ & $1.9\times$ observed ($M_{\mathrm{poly}} = 294$\,TeV, two-loop) \\
...
eq.~\eqref{V-eq:mass-hierarchy}): $m_s/m_b = (v/M_{\mathrm{poly}})^{2\delta_\Delta}/K^2$
with $\delta_\Delta = 0.303$ (the conformal-dimension splitting) and $K = 0.548$.
...
enumerated above.
The binary falsifiable tests are enumerated below
...
\end{itemize}
Among frameworks with zero free parameters, this is the first
to simultaneously predict fermion mass ratios, CKM elements,
