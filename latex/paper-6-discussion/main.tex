...
\item \emph{Restoring:}
  ...
  (bare eigenvalue $-1$, raised to $+3$ by the
  central cyclone).
  $\tau_{\mathrm{restore}}
...
\paragraph{Magnetic white dwarfs.}
Quantised magnetic flux tubes
($\Phi_0 = h/(2e)$) in white dwarfs
($B \sim 10^6$--$10^9$\,G) interact via the
2D logarithmic kernel when the London penetration depth
exceeds the inter-tube spacing.
Ring configurations are limited to $N \le 7$;
