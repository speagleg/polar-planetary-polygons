...
The landscape of possible universes is not the string-theoretic
$10^{500}$ vacuum space.
It is the ADE classification: finitely many types, of which
...
The irreversibility of time follows from the golden ratio:
the frequencies $\varphi$ and $1/\varphi^2$ are incommensurable
with the integer frequencies $1, 2, 3$, so the universe is
\emph{aperiodic}---it can never return to its exact initial state.
The same ``most irrational'' property that makes the icosahedron
most stable (phyllotactic uniformity) makes time irreversible.
