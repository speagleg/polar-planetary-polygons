...
\item \emph{Base genus $g = 2$.}
  ...
  The one-loop $\mathrm{SU}(3)$ running
  $\alpha_s(M_Z) = [1/\alpha_s(M_{\mathrm{poly}})
  - (7/2\pi)\ln(M_{\mathrm{poly}}/M_Z)]^{-1}$
  with $\alpha_s(M_{\mathrm{poly}}) = 1/(4\pi(g{-}1)\sqrt{2})$
  gives $\alpha_s(M_Z) = 0.114$ at $g = 2$,
  consistent with observation ($0.118$).
  At $g = 3$: $\alpha_s(M_Z) = 0.038$ ($68\%$ off).
  Therefore $g = 2$ is the unique genus consistent with
  the derived gauge group and coupling.
\end{enumerate}
...
The golden decoherence theorem (Paper~V, Theorem~V-\ref{V-thm:decoherence})
...
\subsection{The music of the spheres}

The Cartan eigenvalues, interpreted as a musical scale in
$30$-equal temperament ($30$-TET, where $30 = h(E_8)$),
yield an $8$-note scale (rank notes in a Coxeter-equal temperament)
with palindromic interval sequence $6$-$4$-$2$-$3\,|\,3$-$2$-$4$-$6$
(in units of~$\pi/30$).
The palindrome is the musical expression of the bipartite
symmetry: ascending${}={}$matter, descending${}={}$antimatter,
with the Majorana mode (gravity) as the silent turning point.
The interval sizes $\{2,3,4,6\}$ are the divisors of~$12 = h(E_6)$
greater than~$1$.
