...
(\S\ref*{V-sec:mass-hierarchy},
eq.~\eqref{V-eq:mass-hierarchy}): $m_s/m_b = (v/M_{\mathrm{poly}})^{2\Delta c}/K^2$
...
& $N = 11 = 4 + 7$ (Chern class) &
  Exhaustive elimination (\S\ref*{V-sec:n-selection}) \\
& $\mathbf{H}^2$ base &
  Forced by $f(5,11) > N{-}1$ (Prop.~\ref*{IV-prop:spacetime}) \\
...
& Gauge group, 3 generations &
  From $N = 7$ orbifold (Theorem~\ref*{IV-thm:su3}) \\
...
physical requirements (Proposition~\ref*{IV-prop:spacetime}):
