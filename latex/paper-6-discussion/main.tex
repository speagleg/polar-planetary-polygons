...
$\mathrm{SU}(3)$ representation + dynamics & Derived & Theorem & --- \\
$\mathrm{SU}(2) \times \mathrm{U}(1)$ & Derived & Theorem & --- \\
$3$ generations $= (N{-}1)/2$ & Derived & Theorem & --- \\
$4$D graviton (spin $2$) & Derived & Theorem & --- \\
Vacuum Einstein equations & Derived & Theorem & --- \\
...
$\delta_{\mathrm{CKM}} = 70.2^\circ$ & Prediction & Theorem & $0.4\sigma$ \\
...
starting postulate.
Its justification is empirical: the $17$~testable outputs
enumerated above.
...
The master table above classifies each result by role
(Structural / Prediction / Check) and logical status.
The two genuine inputs are $N = 7$ (from the Pell equation,
Theorem~\ref*{V-thm:n-selection}(b))
and $M_P$ (which sets the dimensionful scale).
All structural results, predictions, and consistency checks
in the table follow from these two inputs plus the six
structural identifications detailed in \S\ref{sec:parameters}.
One calibration input ($\Delta m^2_{\mathrm{atm}}$) pins
$M_{\mathrm{poly}} \in [50, 300]$\,TeV.
...
\subsection{Arithmetic connections}

The algebraic structure of the Havelock eigenvalues suggests
a deeper arithmetic interpretation, developed fully in a
...
\emph{vortex stability} $=$ \emph{temperedness} (Langlands)
$=$ \emph{Ramanujan bound}.
For $N = 8$, the automorphic object is the CM form
$\eta(8z)\eta(16z)$ (LMFDB 128.1.d.a),
verified at $46$ primes.

\section{Conclusion}
\label{sec:conclusion}

The Havelock field theory rests on one physical
postulate---that point-vortex polygon dynamics on
$\mathbf{H}^2 \times_N S^1$ is spacetime dynamics---and
one integer, $N = 7$.
From these, the series derives the Standard Model gauge
group, three generations, the CKM phase, the cosmological
constant, the Hubble parameter, and the baryon asymmetry.
The parameter count (two inputs plus six structural
identifications for $17$~testable outputs) compares
favourably to any zero-parameter framework in the literature.
