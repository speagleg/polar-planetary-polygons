...
\item \emph{Base genus $g = 2$.}
  The uniformization theorem requires $g \ge 2$ for constant
  negative curvature ($K = -1$, from~\textbf{A2}).
  The polygon radius $\rho^* \approx 0.51$ is much smaller
  than the injectivity radius of any genus-$2$ surface
  ($\ge 1.53$ for the Bolza surface),
  so the $\mathbb{Z}_7$ acts locally, not as a global isometry,
  and Riemann--Hurwitz places no constraint on~$g$.
  Therefore $g = 2$ is the minimum hyperbolic genus.
\end{enumerate}
...
If the cores of strongly magnetised white dwarfs
