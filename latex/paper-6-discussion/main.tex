...
--- = no empirical target.
$\dagger$ = conditional on the DHVW orbifold construction
at irrational central charge (Paper~IV, \S\ref*{IV-sec:su3-mckay}).
\normalsize
\end{center}
\noindent
...
\end{tabular}
\end{center}
\noindent{\footnotesize $^*$The $\Gamma$-function
evaluation is exact; the systematic from the
Harish-Chandra normalisation assumption
(Paper~IV, Theorem~\ref*{IV-thm:ckm-phase}, Step~4)
is unquantified but structural (binary: the normalisation
...
\emph{(i\hspace{0.5pt}c)~Confinement and $\alpha_s$.}
